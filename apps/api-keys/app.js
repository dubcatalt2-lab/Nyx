...
let ownerKeyTimer=0,ownerKeyVersion=0;
function clearOwnerKey(){clearTimeout(ownerKeyTimer);ownerKeyVersion++;if($('#owner-key-value'))$('#owner-key-value').value='';if($('#owner-key-reveal'))$('#owner-key-reveal').hidden=true;}
...
async function api(path,body,method){const token=await firebaseToken();if(!token){clearOwnerKey();$('#sign-in').hidden=false;$('#create-button').disabled=true;$('#account').textContent='Sign in to Nyx to view your API access.';$('#owner-tab').hidden=true;accountOwner=false;showTab('keys');$('#management').hidden=true;$('#secret').value='';$('#playground-key').value='';$('#reveal').hidden=true;$('#members-rows').replaceChildren();currentUid='';throw Error('Sign in to Nyx to continue.');}const response=await fetch('/api/developer'+path,{method:method||(body?'POST':'GET'),cache:'no-store',headers:{Authorization:'Bearer '+token,'Content-Type':'application/json'},...(body?{body:JSON.stringify(body)}:{})});const data=await response.json();if(!response.ok)throw Error(data.error||'Request failed.');return data;}
...
async function refresh(){const d=await api('/me');if(currentUid!==d.uid){clearOwnerKey();currentUid=d.uid;loadedUid='';$('#secret').value='';$('#playground-key').value='';$('#reveal').hidden=true;$('#members-rows').replaceChildren();$('#uid').value='';}$('#sign-in').hidden=true;$('#account').textContent=`${d.unlimited?"Unlimited":d.balance.toLocaleString()} tokens remaining | ${d.usedTokens||0} used | ${d.dailyRequests} requests/day | ${d.minuteRequests}/minute. User ID: ${d.uid}`;$('#create-button').disabled=Boolean(d.key)||!d.configured;if(d.modelAllowances||d.tokenPool)$('#account').textContent+=' '+modelBalances(d);$('#revoke').hidden=!d.key;$('#key').textContent=d.key?`${d.key.label}: ${d.key.prefix}...`:'No active key.';accountOwner=d.owner;$('#owner-tab').hidden=!d.owner;showTab(location.hash.slice(1));renderUsage(d);syncModels(d);$('#management').hidden=!d.unlocked;if(!d.unlocked)clearOwnerKey();$('#unlock').hidden=Boolean(d.unlocked);if(!$('#uid').value)$('#uid').value=d.uid;if(d.unlocked&&!$('#members-rows').children.length)await loadMembers();}
...
async function loadMembers(cursor=''){const data=await api('/owner/accounts'+(cursor?'?cursor='+encodeURIComponent(cursor):''));const rows=$('#members-rows');rows.replaceChildren();for(const member of data.members||[]){const tr=document.createElement('tr');for(const text of [member.name||member.uid,member.key?.prefix+'...',member.unlimited?'Unlimited':member.balance,member.usedTokens||0]){const td=document.createElement('td');td.textContent=String(text);tr.append(td);}const td=document.createElement('td'),button=document.createElement('button');button.textContent='Manage';button.onclick=task(async()=>{$('#uid').value=member.uid;renderTarget(await api('/owner/account/'+encodeURIComponent(member.uid)));$('#limits').scrollIntoView({block:'center'});});td.append(button);tr.append(td);rows.append(tr);}if(!rows.children.length){const tr=document.createElement('tr'),td=document.createElement('td');td.colSpan=5;td.textContent='No active API keys.';tr.append(td);rows.append(tr);}memberCursor=data.nextCursor;$('#members-next').hidden=!memberCursor;}
$('#members-refresh').onclick=task(()=>loadMembers());$('#members-next').onclick=task(()=>loadMembers(memberCursor));
$('#unlock').onsubmit=task(async e=>{try{await api('/unlock',{password:e.target.elements.password.value});await refresh();await loadMembers();note('Owner management unlocked for 15 minutes.');}finally{e.target.elements.password.value='';}});
$('#lock').onclick=task(async()=>{clearOwnerKey();await api('/lock',{});await refresh();note('Owner management locked.');});
function modelBalances(d){if(d.tokenPool)return `${d.tokenPool.remaining.toLocaleString()} / ${d.tokenPool.limit.toLocaleString()} pooled tokens left; resets ${new Date(d.tokenPool.resetAt).toLocaleDateString()}`;return d.modelAllowances?Object.entries(d.modelAllowances).map(([id,v])=>`${id===LUNA?'Luna':'Gemini'}: ${v.remaining.toLocaleString()} / ${v.limit.toLocaleString()} tokens left`).join(' | '):'';}
function renderTarget(d){clearOwnerKey();loadedUid=d.uid;$('#owner-show-key').disabled=!d.key?.revealAvailable;$('#owner-key-availability').textContent=d.key&&!d.key.revealAvailable?'Older key: only its hash was saved. The user must replace it to enable reveal.':'';$('#target').textContent=`${d.uid}: ${d.unlimited?"Unlimited":d.balance} tokens remaining; ${d.usedTokens||0} tokens used. ${d.key?'Active key: '+d.key.prefix+'...':'No key.'}`;if(d.modelAllowances||d.tokenPool)$('#target').textContent+=' '+modelBalances(d);const f=$('#limits').elements;for(const n of ['dailyRequests','minuteRequests','maxOutput'])f[n].value=d[n];f.addTokens.value=0;for(const [name,model] of [['lunaMonthlyLimit','luna'],['geminiMonthlyLimit','gemini']]){f[name].value=d.monthlyModelLimits?.[model]??0;f[name].disabled=true;f[name].closest('label').hidden=f[name].disabled;}f.gemini.checked=d.models.includes(GEMINI);f.luna.checked=d.models.includes(LUNA);}
$('#uid').oninput=()=>{loadedUid='';};
$('#lookup').onsubmit=task(async()=>renderTarget(await api('/owner/account/'+encodeURIComponent($('#uid').value.trim()))));
$('#limits').onsubmit=task(async e=>{if(!loadedUid)throw Error('Load an account first.');const f=e.target.elements;const body={models:[...(f.gemini.checked?[GEMINI]:[]),...(f.luna.checked?[LUNA]:[])]};for(const n of ['addTokens','dailyRequests','minuteRequests','maxOutput'])body[n]=Number(f[n].value);if(!f.lunaMonthlyLimit.disabled)body.monthlyModelLimits={luna:Number(f.lunaMonthlyLimit.value),gemini:Number(f.geminiMonthlyLimit.value)};renderTarget(await api('/owner/account/'+encodeURIComponent(loadedUid),body));note('Token balance and limits saved.');await refresh();});
async function revealOwnerKey(){
  if(!loadedUid)throw Error('Load an account first.');
  clearOwnerKey();const version=ownerKeyVersion,uid=loadedUid;
  const data=await api('/owner/account/'+encodeURIComponent(uid)+'/reveal-key',{});
  if(version!==ownerKeyVersion||uid!==loadedUid||$('#management').hidden)return;
  $('#owner-key-value').value=data.key;$('#owner-key-label').textContent='API key for '+uid;$('#owner-key-reveal').hidden=false;
  ownerKeyTimer=setTimeout(clearOwnerKey,60000);
}
$('#owner-show-key').onclick=task(revealOwnerKey);
$('#owner-key-copy').onclick=task(async()=>{if(!$('#owner-key-value').value)throw Error('Reveal a key first.');await navigator.clipboard.writeText($('#owner-key-value').value);note('Key copied.');});
$('#owner-key-hide').onclick=clearOwnerKey;
addEventListener('pagehide',clearOwnerKey);document.addEventListener('visibilitychange',()=>{if(document.hidden)clearOwnerKey();});
$('#owner-revoke').onclick=task(async()=>{clearOwnerKey();if(!loadedUid)throw Error('Load an account first.');await api('/owner/account/'+encodeURIComponent(loadedUid)+'/key',null,'DELETE');renderTarget(await api('/owner/account/'+encodeURIComponent(loadedUid)));note('User key revoked.');});
...
document.querySelector('#playground-response').addEventListener('click',async event=>{const button=event.target.closest('[data-copy-code]');if(!button)return;try{await navigator.clipboard.writeText(button.closest('.ai-code-block').querySelector('code').textContent);button.querySelector('span').textContent='Copied';}catch{button.querySelector('span').textContent='Copy failed';}});
