...
function modelBalances(d){return d.modelAllowances?Object.entries(d.modelAllowances).map(([id,v])=>`${id===LUNA?'Luna':'Gemini'}: ${v.remaining.toLocaleString()} / ${v.limit.toLocaleString()} tokens left`).join(' | '):'';}
function renderTarget(d){clearOwnerKey();loadedUid=d.uid;$('#owner-show-key').disabled=!d.key?.revealAvailable;$('#owner-key-availability').textContent=d.key&&!d.key.revealAvailable?'Older key: only its hash was saved. The user must replace it to enable reveal.':'';$('#target').textContent=`${d.uid}: ${d.unlimited?"Unlimited":d.balance} tokens remaining; ${d.usedTokens||0} tokens used. ${d.key?'Active key: '+d.key.prefix+'...':'No key.'}`;if(d.modelAllowances)$('#target').textContent+=' '+modelBalances(d);const f=$('#limits').elements;for(const n of ['dailyRequests','minuteRequests','maxOutput'])f[n].value=d[n];f.addTokens.value=0;for(const [name,model] of [['lunaMonthlyLimit','luna'],['geminiMonthlyLimit','gemini']]){f[name].value=d.monthlyModelLimits?.[model]??0;f[name].disabled=!d.premium||Boolean(d.unlimited);f[name].closest('label').hidden=f[name].disabled;}f.gemini.checked=d.models.includes(GEMINI);f.luna.checked=d.models.includes(LUNA);}
