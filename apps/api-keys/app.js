...
async function refresh(){const d=await api('/me');if(currentUid!==d.uid){currentUid=d.uid;loadedUid='';$('#secret').value='';$('#playground-key').value='';$('#reveal').hidden=true;$('#members-rows').replaceChildren();$('#uid').value='';}$('#sign-in').hidden=true;$('#account').textContent=`${d.unlimited?"Unlimited":d.balance.toLocaleString()} tokens remaining | ${d.usedTokens||0} used | ${d.dailyRequests} requests/day | ${d.minuteRequests}/minute. User ID: ${d.uid}`;$('#create-button').disabled=Boolean(d.key)||!d.configured;$('#revoke').hidden=!d.key;$('#key').textContent=d.key?`${d.key.label}: ${d.key.prefix}...`:'No active key.';accountOwner=d.owner;$('#owner-tab').hidden=!d.owner;showTab(location.hash.slice(1));renderUsage(d);syncModels(d);$('#management').hidden=!d.unlocked;$('#unlock').hidden=Boolean(d.unlocked);if(!$('#uid').value)$('#uid').value=d.uid;if(d.unlocked&&!$('#members-rows').children.length)await loadMembers();}
...
function renderTarget(d){loadedUid=d.uid;$('#target').textContent=`${d.uid}: ${d.unlimited?"Unlimited":d.balance} tokens remaining; ${d.usedTokens||0} tokens used. ${d.key?'Active key: '+d.key.prefix+'...':'No key.'}`;const f=$('#limits').elements;for(const n of ['dailyRequests','minuteRequests','maxOutput'])f[n].value=d[n];f.addTokens.value=0;f.monthlyTokenLimit.value=d.premium&&!d.unlimited?d.monthlyTokenLimit??50000:0;f.monthlyTokenLimit.disabled=!d.premium||Boolean(d.unlimited);f.monthlyTokenLimit.closest('label').hidden=f.monthlyTokenLimit.disabled;f.gemini.checked=d.models.includes(GEMINI);f.luna.checked=d.models.includes(LUNA);}
$('#uid').oninput=()=>{loadedUid='';};
$('#lookup').onsubmit=task(async()=>renderTarget(await api('/owner/account/'+encodeURIComponent($('#uid').value.trim()))));
$('#limits').onsubmit=task(async e=>{if(!loadedUid)throw Error('Load an account first.');const f=e.target.elements;const body={models:[...(f.gemini.checked?[GEMINI]:[]),...(f.luna.checked?[LUNA]:[])]};for(const n of ['addTokens','dailyRequests','minuteRequests','maxOutput'])body[n]=Number(f[n].value);if(!f.monthlyTokenLimit.disabled)body.monthlyTokenLimit=Number(f.monthlyTokenLimit.value);renderTarget(await api('/owner/account/'+encodeURIComponent(loadedUid),body));note('Token balance and limits saved.');await refresh();});
