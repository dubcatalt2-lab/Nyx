...
function renderTarget(d){loadedUid=d.uid;$('#target').textContent=`${d.uid}: ${d.unlimited?"Unlimited":d.balance} tokens remaining; ${d.usedTokens||0} tokens used. ${d.key?'Active key: '+d.key.prefix+'...':'No key.'}`;const f=$('#limits').elements;for(const n of ['dailyRequests','minuteRequests','maxOutput'])f[n].value=d[n];f.addTokens.value=0;f.monthlyTokenLimit.value=d.monthlyTokenLimit??50000;f.gemini.checked=d.models.includes(GEMINI);f.luna.checked=d.models.includes(LUNA);}
$('#uid').oninput=()=>{loadedUid='';};
$('#lookup').onsubmit=task(async()=>renderTarget(await api('/owner/account/'+encodeURIComponent($('#uid').value.trim()))));
$('#limits').onsubmit=task(async e=>{if(!loadedUid)throw Error('Load an account first.');const f=e.target.elements;const body={models:[...(f.gemini.checked?[GEMINI]:[]),...(f.luna.checked?[LUNA]:[])]};for(const n of ['monthlyTokenLimit','addTokens','dailyRequests','minuteRequests','maxOutput'])body[n]=Number(f[n].value);renderTarget(await api('/owner/account/'+encodeURIComponent(loadedUid),body));note('Token balance and limits saved.');await refresh();});
