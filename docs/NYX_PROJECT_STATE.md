...
- Latest application release: commit `9d5baf3` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-13. NyxCloud and its SoundCloud credential surface are retired; its app redirects home, its former API returns `410`, and old saved shortcuts migrate away. NyxTube and Nyxify use the shared Echo-inspired responsive visual language without copying Echo branding. ChromeOS defaults NyxTube to YouTube's standard official embed with a saved privacy-player toggle. Nyxify audio traverses the fixed-target same-origin `/api/music/stream/:trackId` relay; the relay primes browsers that omit `Range` with a bounded first chunk and preserves exact follow-up byte ranges. Production verification confirmed health, configured NyxTube search and standard ChromeOS playback, a `206 audio/mpeg` relay response, and error-free Nyxify audio reaching ready state 4 and advancing. The canonical homepage also includes the complete StudyHub search and social metadata.
- Unreleased working tree: no tracked application changes after the release record. The protected untracked local work directories remain present and untouched.
