...
- Latest application release: commits `35fd455` and `678bb01` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-14. The release adds per-tab proxy cookie isolation and cleanup, precise-geolocation denial, an Aether-backed Movies app, key-authorized Nocturne AI model discovery, and local KaTeX plus Markdown/TSV table rendering. The live key currently authorizes 23 models; Claude Opus 5 is not exposed because Nocturne rejects the known Claude Opus IDs for this key.
