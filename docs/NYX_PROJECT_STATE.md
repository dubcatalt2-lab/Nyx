...
- Latest application release: commit `fec4740` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-20. It preserves the account-controls and home-polish release from `d4b91b8`, keeps the real external address when a proxied frame reports `/unidentified` or an internal transport path, provides a Nyx-owned right-click menu that copies clean external links, protects the Owner Dashboard from escaped-ad quarantine, adds Discord-style Chat emoji suggestions, and keeps unchanged Chat message nodes mounted during realtime updates. The standalone `nyx.html` redirect is ready for an optional Google Cloud Storage upload; no Google bucket or Google-hosted link has been provisioned.
