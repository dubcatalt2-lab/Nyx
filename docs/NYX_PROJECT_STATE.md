...
- Latest application release: commit `564c00a` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-20. It refreshes GN Math from the current upstream catalog; retires Nyxify and its provider APIs; places Movies in its former homepage slot; adds owner-managed global apps, More Movie Sites, the OVH-backed single-file download, and homepage utility links; strengthens browser/ad isolation; and keeps the Fern constellation background visible at medium performance with built-in and custom theme colors.
