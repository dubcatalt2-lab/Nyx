...
- Latest application release: commit `8d1c809` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-20. Chat now recognizes 158 common Discord-style Unicode emoji shortcode names and aliases, shows them in the existing composer suggestions, and cache-versions the updated Chat bundle. The preceding `a2fc00b` release added the three-second homepage latency chart and health details, keeps theme-colored constellations visible under custom themes and Lag Reducer, places Home before the URL field and Settings after Developer, preserves Claude model selection, sends real image bytes to vision models through short-lived in-memory public URLs, retries corrupted symbol-only AI output once, wraps attachment errors, and keeps the downloaded single-file launcher on its local `file:///` address while embedding the canonical OVH release. Link Generator's source fallback now uses `https://nyxlearning.org` rather than the retired Netlify hostname.
...
- Chat expands a local Discord-style catalog of common Unicode emoji shortcodes and aliases such as `:smiley:`, `:joy:`, `:skull:`, `:heart:`, `:white_check_mark:`, and `:coffee:` on the actual send path and shows matching suggestions as the member types. Nyx does not yet support server-specific custom emoji; unknown shortcodes remain unchanged.
