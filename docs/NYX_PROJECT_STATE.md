...
- Latest application release: commit `9a0019a` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-21. Active Premium accounts receive 50,000 Firestore-backed Claude Opus AI credits per UTC day, and the original `Nyx-Download.html` launcher again embeds the complete online Nyx application, including Pirate Cove. The release baseline retains exact external `/unidentified` recovery, oversized AI text attachments, guarded one-second homepage health probes, the provider-verified Claude Sonnet 4.5 model alongside DeepSeek V4 Flash, per-tab in-site browser history, Link Checker blocker details and rate-limit handling, current Chat commands and emoji catalog, theme-aware constellations, verified vision upload, and OVH Link Generator fallback.
...
- The redesigned homepage uses a sparse, reduced-motion-aware constellation field on a dark theme-derived canvas, with the same compact hero geometry across themes. Theme choices tint that canvas and provide the interaction accent, while normal text, navigation icons, fields, and panels retain a readable neutral white/gray/black hierarchy. The same balance applies to the generated Apps and Settings workspaces. The particle wordmark owns the visible **NYX** title after its canvas initializes; theme text rules must not expose the solid fallback letters underneath it. **Home design** in Settings can restore the maintained original homepage, which disables redesign-only styles and behavior rather than leaving parts of the new shell behind. The browser tab drawer must remain reachable above Settings so that Settings never traps navigation. The redesigned homepage and Apps workspace must fit without horizontal or vertical clipping down to a 320-pixel-wide viewport. The Apps shortcut opens the complete Apps tab directly; the retired hidden app-launcher drawer and its duplicate catalog markup must not be restored.
- The redesigned homepage background follows the Fern star-theme composition: a dark surface derived from the active built-in or custom theme, a denser field of tiny stars, and five to seven larger edge-positioned constellation webs built from nearest-neighbor links on normal desktop viewports. A black custom theme resolves to a true jet-black canvas with white constellation nodes. Narrow screens and medium performance mode use fewer groups to preserve space and rendering headroom. The stars and connecting lines derive their visible color from the active built-in or custom theme and repaint immediately when the theme changes; constellation nodes use a brighter theme-relative color so they remain legible over every canvas. The lightweight Fern canvas remains the redesigned home's background even when the legacy 3D-background preference is saved. Medium and minimum/Lag Reducer modes keep a lower-density static field visible while disabling its pointer motion and other heavier effects.
