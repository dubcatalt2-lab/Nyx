...
- Chat member lists keep online and offline groups separate, then sort each group by the canonical Nyx role rank (Owner through Member) and display name. Moderator-or-higher roles can open **Flagged searches** from the member sidebar or use the shield beside an account in Owner Dashboard to review that account's retained matches. Nyx never submits or stores ordinary searches: the browser sends only authenticated explicit search text that matches a conservative inappropriate-content classifier, the server classifies it again before writing, and direct Firestore access to `nyxFlaggedSearches` is denied. Records contain the search text, classification, account identity, role, and timestamp; they do not contain IP addresses, destination URLs, clicked results, or search-engine metadata. Records expire after 30 days and are exposed only through the authenticated Moderator-or-higher Chat API. The configured Owner can review all retained flagged searches, while every non-owner reviewer receives a server-filtered result that excludes records belonging to the configured Owner UID or carrying the Owner role; the dashboard also hides the Owner shield from non-owner staff.
