...
- Latest application release: commit `a2d62fb` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-18. The release carries the balanced minimalist homepage/theme system, live Performance dashboard metrics, refreshed Owner Dashboard and Chat presentation, account-action notices, protected profile-media resolution, and the current browser/settings safeguards. Content-toolbar glyphs remain neutral white under every theme, while the temporarily unsupported Home Design selector is hidden without removing the original-layout compatibility path.
...
- 2026-08-18 balanced-interface and account-safeguard release: OVH fast-forwarded to `a2d62fb`, rebuilt the VPS bundle, passed deployment and branding checks, restarted Nyx, and validated Caddy and TURN. Production Chromium opened Settings under Sakura with all eight toolbar glyphs rendered neutral white, no visible Home Design selector, no horizontal overflow, and no page errors. Public and local health reported embedded Wisp plus Socket.IO Chat, `nyx`, `caddy`, and `coturn` were active, and the compiled Firestore rules were released to `lionrunner` with direct access to `nyxAccountNotices` denied.
