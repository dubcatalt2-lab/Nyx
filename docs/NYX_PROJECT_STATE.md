...
- Latest application release: commit `14408e6` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-13. Owner Dashboard custom-role controls now fit narrow drawers using container-aware layout, search-history shields use explicit server-authorized per-user capability data instead of displayed role labels, and the Owner role remains Owner during role presentation. Tide privacy and actual permissions remain unchanged.
