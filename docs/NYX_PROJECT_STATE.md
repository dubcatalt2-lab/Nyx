...
- Latest application release: commit `5f38431` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-14. The release removes Nyxify's generated Popular Tracks shelf, its client-side discovery state and automatic `global hits` request, and the matching server-start audio warmup. Artists now derive only from real device listening history and stay hidden until history exists; search, user-selected mixes, library, queue, and playback remain available. The earlier rose-profile, AI display-name, private-tab, Movies, AI model-discovery, KaTeX, and Markdown/TSV work remains part of the release baseline. The live key currently authorizes 23 models; Claude Opus 5 is not exposed because Nocturne rejects the known Claude Opus IDs for this key.
