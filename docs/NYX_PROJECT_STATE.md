...
- Unreleased working tree: search-history recording now waits for Firebase to finish restoring the signed-in account, retries once with a refreshed token after an authentication rejection, and displays the retention disclosure only after the server confirms the write. The protected untracked local work directories remain present and untouched.
