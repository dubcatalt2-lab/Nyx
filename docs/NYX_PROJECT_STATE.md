...
- Latest application release: commit `9170da8` on `agent/pirate-cove`, deployed to OVH on 2026-08-11. Nyx Chat groups the member sidebar by canonical role rank, gives every non-Member role a readable continuously traveling cyan/green/violet username wave with a tight low-opacity halo, and grants Owner and Co-owner permanent Caffeine with unlimited distinct gifts. It preserves search-history recording, the scoped TikTok Ultraviolet compatibility path, louder Chat notifications, persistent timed Chat mutes, Founder Owner moderation hierarchy overrides, Firebase-authenticated Socket.IO delivery, voice signaling and coturn, normal subscription and recipient Caffeine rules, private DMs, attachments, reactions, mentions, role-restricted channels, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing.
- Unreleased working tree: every special-role username now uses a distinct readable palette: pink Owner, gold Co-owner, red Admin, purple Manager, blue Developer, green Moderator, teal Support, orange Tester, and silver Contributor. Names remain static normally; their color wave starts only while that username is hovered and resets on pointer exit. The low-opacity 1–2 px halo and existing accessibility fallbacks remain unchanged. The protected untracked local work directories remain present and untouched.
