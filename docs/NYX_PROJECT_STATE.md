...
- Latest application release: commit `dd6b3f0` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-18. The release carries the balanced minimalist homepage/theme system, live Performance dashboard metrics, refreshed Owner Dashboard and Chat presentation, account-action notices, protected profile-media resolution, the corrected Chat **Back to home** handoff, and server-authorized `/warn` and `/demote` commands. Content-toolbar glyphs remain neutral white under every theme, while the temporarily unsupported Home Design selector is hidden without removing the original-layout compatibility path.
