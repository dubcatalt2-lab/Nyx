...
- Latest application release: commit `341dacb` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-18. It retains the balanced minimalist homepage/theme system, live Performance dashboard metrics, refreshed Owner Dashboard and Chat presentation, account-action notices, protected profile-media resolution, corrected Chat navigation, server-authorized moderation commands, personal Nyx AI key/preferences handoff, and exact-name search metadata from the prior release. Nyxify now uses a compact chart-first layout with a narrow navigation rail, restrained top bar, ranked catalog rows, fixed player, and responsive library/queue surfaces while preserving the existing same-origin search, playback, likes, history, and playlist flows.
