...
- Latest release: Nyx Chat Caffeine gifting, clipboard attachment paste, role-restricted text/voice channels, regular 8 MB and Owner-only streamed 1 GB attachments, a Chat browser-menu entry, improved voice gain, and voice continuity across Nyx tab changes, plus the existing private messaging, reactions, mentions, parent-tab alerts, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing on `agent/pirate-cove`
...
- Nyx Chat uses the same narrowly scoped parent token bridge, with the parent accepting only the exact `/apps/chat/` and `/apps/link-checker/` paths. Chat is unavailable to guests. Staff-managed community channels and two-participant private DMs read and write through authenticated `/api/chat/*` routes; direct browser access to `nyxChatChannels`, `nyxChatConversations`, `nyxChatAttachments`, `nyxCaffeineGifts`, and `nyxChatConfiguration` remains denied by Firestore rules. Owner, Co-owner, Admin, and Manager roles can create, rename, remove, and apply minimum-role restrictions to text or voice channels through the server API; at least one channel of each kind is retained. The default **Staff Room** requires Moderator or above, and server-side scope checks protect its messages, attachments, activity, presence, and voice signaling. Private-message APIs and attachment downloads verify that the requester is a conversation participant, and public-channel moderator powers do not grant access to private DMs. Scope resolution must test explicit channel and conversation identifiers without allowing an absent channel to default to `general`; this keeps DM reads, sends, reactions, deletes, and attachment authorization isolated from public chat. Messages support plain text up to 1,000 characters, up to three allowlisted 8 MB attachments with an 8 MB combined limit, and the fixed reaction palette. Allowlisted attachments include the existing image, PDF, text, and Office formats plus common MP3/M4A/OGG/WAV/WebM audio and MP4/MOV/OGG/WebM video; media renders with authenticated in-chat controls and unsafe active web/executable formats remain blocked. Regular attachments upload in bounded Firestore chunks and are served only through authenticated routes, and files or images pasted into the composer with Ctrl+V follow the same validation. The Owner role alone can upload supported files larger than 8 MB and up to 1 GB, with a 1 GB combined message limit; those uploads use resumable 4 MB binary chunks, OVH disk storage under `/var/lib/nyx/chat-attachments`, short-lived authenticated download tickets, HTTP range responses, a 1 GB free-space reserve, and expired-upload cleanup. The systemd unit grants write access only to that state directory, and message deletion also removes the disk file. Authors can delete their own messages, while Moderator-or-higher roles can remove others' public-channel messages. Member mentions autocomplete from the current member list; only Moderator-or-higher roles may send `@everyone`. The client polls active messages and lightweight conversation/channel activity separately and stores only unread timestamps on the device. New incoming DMs or mentions send a narrowly validated same-origin notification to the parent Nyx tab, which plays one two-tone ping after browser audio is unlocked and displays a red unread dot until that tab is activated; standalone Chat retains the local sound. Clicking a chat member asks the same-origin Nyx parent to open that account in the existing full public-profile viewer.
- **Caffeine** is the chat-facing name for the existing Nyx Premium entitlement. Directly assigned Premium or Trial accounts may send one Caffeine gift for the current subscription grant. The recipient sees an in-chat acceptance prompt; accepting atomically marks the gift accepted and grants the existing Premium subscription status. Gift-derived Premium cannot be gifted again, an active recipient cannot accept another gift, pending gifts expire after seven days, and Owner Dashboard subscription assignments rotate the gift grant only when beginning a new Premium period rather than on every save. There is no separate payment processor or checkout record in this repository, so “purchased” currently means Premium or Trial assigned through the existing administration flow.
- Nyx Chat voice rooms are account-only and use the server-managed voice-channel list, initially **Lounge**, **Gaming**, and **Study Room**. Audio travels directly between browsers with WebRTC and is never recorded or stored by Nyx; the persistent OVH Node process keeps only short-lived authenticated room presence and offer/answer/ICE signaling in memory. Sessions expire after five minutes without polling so hidden-frame throttling cannot immediately eject callers, signaling expires after 60 seconds, and each room is limited to eight participants to bound browser CPU/network use. The client provides join/leave, microphone mute, deafen, live participant lists, and profile handoff. Voice capture uses echo cancellation, noise suppression, and microphone automatic gain control; received audio uses gain plus compression, prefers Opus with in-band FEC and DTX disabled, requests a modest receive jitter buffer, prioritizes the audio sender, and restarts ICE after sustained disconnection. Nyx's internal tab switcher must leave Chat audio running instead of pausing its media elements. Public STUN is the default; networks that require TURN can still prevent peers from connecting, and the in-memory signaling layer is intentionally designed for the active single-instance OVH deployment rather than serverless multi-instance hosting.
...
- `NYX_CHAT_ATTACHMENT_ROOT` (OVH only; defaults to `/var/lib/nyx/chat-attachments` in the deployment template)
- `NYX_PROJECT_ROOT`
...
- Voice channels use browser-to-browser WebRTC with public STUN and no TURN relay. Restrictive school/corporate networks or symmetric NAT can prevent audio connections even though room presence works; add a properly secured TURN service before treating voice as universally reachable.
- Owner large chat attachments consume the VPS's local 40 GB disk and are not copied to external object storage. Nyx preserves a 1 GB free-space reserve and removes a disk object when its message is deleted, but production operations should still monitor `/var/lib/nyx/chat-attachments` and VPS disk usage.
