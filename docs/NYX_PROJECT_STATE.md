...
- Unreleased working tree: signed-in searches made through Nyx are now retained for 30 days and exposed as Search history to authorized Moderator-or-higher reviewers, with classifier matches highlighted. Link Checker's device-local FreeDNS registry and verdict cache now expires together after eight inactive hours, and verdict persistence uses a compact registry-indexed format to avoid the local-storage quota failure seen around 15,600 results. The protected untracked local work directories remain present and untouched.
...
- Chat member lists keep online and offline groups separate, then sort each group by the canonical Nyx role rank (Owner through Member) and display name. Moderator-or-higher roles can open **Search history** from the member sidebar or use the shield beside an account in Owner Dashboard to review that account's retained searches. Every explicit search made through Nyx while an account is signed in is sent to the authenticated server route and retained in `nyxSearchHistory` for 30 days; the first signed-in search on a browser displays a disclosure toast. The conservative classifier remains as a server-generated label so policy-matching searches are highlighted, but normal searches are retained too. Records contain the search text, classification, account identity, role, and timestamp; they do not contain IP addresses, destination URLs, clicked results, or search-engine metadata. Direct Firestore access is denied, and results are exposed only through the authenticated Moderator-or-higher API. The configured Owner can review all retained searches, while every non-owner reviewer receives a server-filtered result that excludes records belonging to the configured Owner UID or carrying the Owner role; the dashboard also hides the Owner shield from non-owner staff. Legacy flagged-search API paths remain aliases during the cached-client transition.
