...
- Latest application release: commit `aed0c07` on `agent/pirate-cove`, deployed to OVH on 2026-08-11. Search-history recording now waits for Firebase to restore the signed-in account, retries once with a refreshed token after an authentication rejection, and displays the retention disclosure only after the server confirms the write. It preserves the scoped TikTok Ultraviolet compatibility path, louder Chat notifications, persistent timed Chat mutes, Founder Owner moderation hierarchy overrides, Firebase-authenticated Socket.IO delivery, voice signaling and coturn, Caffeine gifting, private DMs, attachments, reactions, mentions, role-restricted channels, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing.
- Unreleased working tree: Nyx Chat gives every non-Member role a Discord-inspired layered role-color name treatment with a tight glow and subtle moving highlight across messages, DMs, member lists, profiles, mentions, voice participants, and gift pickers. The member sidebar is grouped by canonical role rank from Owner through Member, with online accounts first inside each role. Member names remain neutral; reduced-motion mode stops the highlight and increased-contrast mode restores solid role colors. The protected untracked local work directories remain present and untouched.
