...
- Latest application release: commit `ffeb82f` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-15. Nyx AI now supports request-isolated personal Nocturne keys, per-key verified catalogs including current Claude candidates, searchable chats, native Off/Normal/Extended response depth, current-model display, and local estimated usage totals. The live shared key currently authorizes DeepSeek V4 Flash and Claude Opus 4.8. The search-discovery foundation, StudyHub tab identity, Nyxify feed removal, rose-profile, private-tab, Movies, KaTeX, Markdown/TSV, and earlier release behavior remain part of the baseline.
