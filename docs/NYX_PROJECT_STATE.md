...
- Latest application release: commit `42aa867` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-15. Nyx now exposes its canonical search-discovery foundation through homepage `WebSite`/`Organization` metadata, a public About Nyx page, `robots.txt`, and `sitemap.xml`, while the browser tab continues to present StudyHub from first paint through the completed startup transition. The earlier StudyHub tab migration, Nyxify feed removal, rose-profile, AI display-name, private-tab, Movies, AI model-discovery, KaTeX, and Markdown/TSV work remains part of the release baseline. The live key currently authorizes 23 models; Claude Opus 5 is not exposed because Nocturne rejects the known Claude Opus IDs for this key.
