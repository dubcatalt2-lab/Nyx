...
- Latest application release: commit `3646da8` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-14. The release makes the rose profile decoration visibly grow and open eight individually staggered blooms whenever a profile card is mounted, preserves a completed static frame for reduced-motion users, and renders Minecraft-formatted display names in the Nyx AI profile control without exposing raw formatting codes. The earlier private-tab, Movies, AI model-discovery, KaTeX, and Markdown/TSV work remains part of the release baseline. The live key currently authorizes 23 models; Claude Opus 5 is not exposed because Nocturne rejects the known Claude Opus IDs for this key.
