...
Last repository review: 2026-08-22
...
- Latest application release: commit `107cca2` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-22. Public root-page link previews now use StudyHub-only titles, descriptions, and a dedicated local StudyHub PNG. The redesigned homepage keeps a pure-black base canvas for every built-in and custom theme while constellation dots and links remain bright and theme-relative; a black custom theme produces white nodes. The release baseline retains 50,000 daily Firestore-backed Claude Opus AI credits for active Premium accounts, the complete original `Nyx-Download.html` launcher, exact external `/unidentified` recovery, oversized AI text attachments, guarded one-second homepage health probes, Claude Sonnet 4.5 alongside DeepSeek V4 Flash, per-tab in-site browser history, Link Checker blocker details and rate-limit handling, current Chat commands and emoji catalog, verified vision upload, and OVH Link Generator fallback.
...
- The redesigned homepage uses a sparse, reduced-motion-aware constellation field on a pure-black canvas, with the same compact hero geometry across themes. Theme choices provide the constellation and interaction accents without tinting the base canvas, while normal text, navigation icons, fields, and panels retain a readable neutral white/gray/black hierarchy. The same balance applies to the generated Apps and Settings workspaces. The particle wordmark owns the visible **NYX** title after its canvas initializes; theme text rules must not expose the solid fallback letters underneath it. **Home design** in Settings can restore the maintained original homepage, which disables redesign-only styles and behavior rather than leaving parts of the new shell behind. The browser tab drawer must remain reachable above Settings so that Settings never traps navigation. The redesigned homepage and Apps workspace must fit without horizontal or vertical clipping down to a 320-pixel-wide viewport. The Apps shortcut opens the complete Apps tab directly; the retired hidden app-launcher drawer and its duplicate catalog markup must not be restored.
...
- Aether navigation is always forced through Nyx's contained popup bridge, even when general popup protection is disabled. Nyx also quarantines Aether's known escaped download-reminder overlay and cleans up any remainder when users switch or close tabs, so third-party ad UI cannot replace or cover the Nyx shell.
...
Nocturne's Claude Opus 4.8 endpoint currently rejects image content even though it accepts text. Claude must not be advertised as vision-capable unless the provider later reports that capability. When a user attaches an image while a text-only model is selected, the AI workspace may use the verified `nocturne:flash` vision model for that one request while leaving the user's selected and saved text model unchanged.
...
AI image attachments use the selected provider model's actual vision input rather than relying on local OCR or average-color summaries. The browser preserves supported images that already fit the bounded request and otherwise downsizes/compresses them before sending them only to Nyx's same-origin AI route. The server validates the upload, holds at most twelve bounded images in memory, and gives the provider a random 256-bit HTTPS capability URL that expires after 90 seconds; image bytes are never written to disk, placed in source, or logged. This is required because Nocturne's API-key endpoint accepts HTTPS `image_url` parts but silently drops inline `data:` image URLs. The model picker marks vision-capable entries, preserves the user's selected model, and requires an explicit model choice when the current model cannot read images. Temporary catalog gaps use an available fallback without overwriting the saved model preference, so the preferred model is restored when it returns. Stream decoding flushes split UTF-8 data correctly, normalizes structured provider deltas, and replaces a detected symbol-corrupted response with one low-temperature retry or a readable failure instead of retaining gibberish.
...
- The Chat slash-command registry contains 82 commands: the existing messaging, voice, Caffeine, information, and server-authorized staff actions plus 50 member-safe utilities for text formatting, encoding, random choices, calculations, timers, dates/time zones, channel/member summaries, status messages, and welcomes. `/warn @person reason` sends an audited official warning through a private conversation, and `/demote @person` moves an eligible target down one built-in role through the same server-enforced hierarchy used by the Owner Dashboard. Commands remain discoverable through `/help` and composer autocomplete; generated messages are subject to the same 1,000-character ceiling as manually typed messages.
...
- Successful dashboard mutations update the current session immediately through `nyx:global-apps-changed`; other visible sessions refresh `/api/apps` at most one minute later. Every generated Apps surface uses the same in-memory catalog. App tiles have a restrained lift/scale hover and focus animation, disabled by the user's reduced-motion preference.
