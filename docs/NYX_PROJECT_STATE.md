...
- Unreleased working tree: Discord-style message context actions, applicable slash commands with server-enforced moderation, long-lived voice presence, optional authenticated coturn relay support, role-ranked member lists, a privacy-limited flagged-search review for Moderator-or-higher roles, Firestore quota-pressure safeguards, a uniform 8 MB Chat attachment cap, and Firebase-authenticated Socket.IO delivery for live Chat and voice signaling with low-frequency consistency fallbacks. These changes have not been committed, pushed, or deployed.
...
- Voice calls fall back to public STUN when `NYX_TURN_URLS` and `NYX_TURN_SHARED_SECRET` are absent. Restrictive networks can still show room presence without carrying audio until the authenticated coturn relay is configured on OVH and its `3478` plus bounded relay ports are allowed through every active firewall.
- The flagged-search classifier is intentionally conservative and can still produce false positives or miss disguised harmful language. It is a moderation signal, not proof of wrongdoing; ordinary search history must remain uncollected.
