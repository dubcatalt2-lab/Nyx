...
- Latest application release: commit `0357b23` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-21. Link Checker's saved FreeDNS vendor shield strip opens detailed blocker results even while a wider registry scan is running. Chat members can change their locally saved command prefix from `/` to one or two non-alphanumeric symbols through the command list; command autocomplete and the displayed command usage follow the selected prefix. Moderators can use `/tempban`, `/untempban`, `/duration`, and `/tempbans` (or `/moderations`), while `/timeout` aliases the existing temporary Chat mute. Temporary bans are stored server-side, revoke sessions, enforce the existing role hierarchy, expire automatically, and are cleared by permanent account actions. Chat also loads a local catalog of 1,948 standard Unicode emoji names and aliases, including corrected `:cold_face:` and skull-and-crossbones mappings; reactions and emoji-only messages use larger styling. The preceding `a2fc00b` release added the three-second homepage latency chart and health details, keeps theme-colored constellations visible under custom themes and Lag Reducer, places Home before the URL field and Settings after Developer, preserves Claude model selection, sends real image bytes to vision models through short-lived in-memory public URLs, retries corrupted symbol-only AI output once, wraps attachment errors, and keeps the downloaded single-file launcher on its local `file:///` address while embedding the canonical OVH release. Link Generator's source fallback now uses `https://nyxlearning.org` rather than the retired Netlify hostname.
