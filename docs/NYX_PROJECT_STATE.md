...
- The redesigned homepage background follows the Fern star-theme composition: a charcoal-black surface, a denser field of tiny stars, and five to seven larger edge-positioned constellation webs built from nearest-neighbor links on normal desktop viewports. Narrow screens and medium performance mode use fewer groups to preserve space and rendering headroom. The stars and connecting lines derive their visible color from the active built-in or custom theme and repaint immediately when the theme changes. The lightweight Fern canvas remains the redesigned home's background even when the legacy 3D-background preference is saved. Medium performance mode keeps a lower-density static field visible, while minimum mode may hide it as an emergency safeguard.
- The redesigned homepage replaces the former centered dashboard with a small, non-interactive live latency bubble. It measures the browser-to-Nyx round trip through cache-bypassed same-origin `/healthz` requests, calibrates from three initial probes, then updates a five-sample rolling median every ten seconds while the page is visible. This rejects connection-warmup and event-loop outliers while retaining a current quality signal. The bubble shows the stabilized milliseconds with a quality color and reports **Offline** when the health route fails, times out, or the browser loses connectivity. The separate authenticated Owner Dashboard remains unchanged.
...
- Single-file export/download behavior; the Settings action fetches the current launcher into a completed browser Blob before saving `Nyx-Download.html`, `/download/nyx-singlefile.html` remains a direct attachment route, and the launcher opens the canonical OVH-hosted `https://nyxlearning.org/` release rather than the retired Netlify host
