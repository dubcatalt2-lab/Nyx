...
- `apps/nyxtube/`, `apps/nyxcloud/`, and `apps/media-browser.*`: separate YouTube and SoundCloud search/player workspaces backed by same-origin server APIs
...
- NyxTube and NyxCloud search/playback through official provider APIs and embedded players, plus Nyxify search/playback through the public Meting-compatible service
...
- NyxTube calls Nyx's same-origin `/api/nyxtube/*` bridge, searches only embeddable videos through YouTube Data API v3, and plays them with the official privacy-enhanced IFrame player. NyxCloud calls `/api/nyxcloud/*`, obtains and reuses a server-only SoundCloud client-credentials token, searches playable public tracks, and plays them with SoundCloud's official widget. Provider credentials remain only in `/etc/nyx/nyx.env`; callers are rate-limited and identical searches are cached briefly. These apps must not extract raw YouTube media URLs or expose SoundCloud secrets.
- Nyxify is a distinct music workspace at `/apps/nyxify/`. Its same-origin `/api/nyxify/*` bridge searches the documented public Meting-compatible endpoint, validates the returned `api.qijieya.cn/meting/` asset URLs, and requires no API key. Its playback queue, shuffle/repeat state, recent history, liked songs, and playlists are device-local. It must remain branded as Nyxify rather than pretending to be Spotify, and the upstream community service must be treated as an availability risk.
...
- NyxTube search depends on Google project quota, while NyxCloud search depends on SoundCloud app eligibility, OAuth availability, and each track's off-platform playback permission. Nyxify depends on the community-operated `api.qijieya.cn` Meting service, which has no guaranteed availability or service-level agreement. All three apps fail independently.
