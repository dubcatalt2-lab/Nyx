...
- Latest application release: commit `1b88728` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-13. The Tide custom role is private to the configured Owner and the assigned Tide account; every other viewer receives a Moderator presentation with no Tide role metadata across profiles, Chat/bootstrap/messages/DMs/voice/realtime events, search-history moderation, and dashboard user records. Actual Tide permissions and hierarchy remain unchanged server-side.
