...
- Latest application release: commit `462a670` on `agent/pirate-cove`, deployed to OVH on 2026-08-11. Nyx Chat groups the member sidebar by canonical role rank, gives every non-Member role a continuously traveling multicolor username glow wave, and grants Owner and Co-owner permanent Caffeine with unlimited distinct gifts. It preserves search-history recording, the scoped TikTok Ultraviolet compatibility path, louder Chat notifications, persistent timed Chat mutes, Founder Owner moderation hierarchy overrides, Firebase-authenticated Socket.IO delivery, voice signaling and coturn, normal subscription and recipient Caffeine rules, private DMs, attachments, reactions, mentions, role-restricted channels, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing.
- Unreleased working tree: Chat's animated special-role username wave uses darker saturated cyan/green/violet bands with a much tighter, lower-opacity halo so names remain readable at the member sidebar's 11 px size. The wave timing, role coverage, reduced-motion behavior, and increased-contrast fallback remain intact. The protected untracked local work directories remain present and untouched.
