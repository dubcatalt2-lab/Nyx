...
- Link Checker vendor scans go through Nyx's same-origin `/api/link-checker/*` server bridge to `lc.nocturne.lol`. Individual checks add `NYX_LINK_CHECKER_API_KEY` only when that optional server variable is configured. Fast full-registry scans use Nocturne's authenticated server-side scan queue through `NYX_LINK_CHECKER_ACCOUNT_USERNAME` and `NYX_LINK_CHECKER_ACCOUNT_PASSWORD`; Nyx keeps the resulting session cookie only in server memory and automatically signs in again after rejection or restart. Never put any of these values in client code, tracked files, documentation, logs, or commits.
