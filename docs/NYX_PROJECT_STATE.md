...
- Latest application release: commit `ab308a5` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-21. Link Checker's saved FreeDNS vendor shield strip opens detailed blocker results while wider scans run. Ordinary lookups use the standard checker API rather than the shared Nocturne account; account-only page and full-registry work has constrained concurrency and honors upstream retry delays. The release baseline retains the current Chat commands and emoji catalog, homepage latency chart and theme-aware constellations, verified vision upload path, local single-file launcher, and OVH Link Generator fallback.
