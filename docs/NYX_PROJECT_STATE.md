...
- Unreleased working tree: NyxTube is retired from the source application, home shortcuts, Apps list, deployment checks, environment template, and active media APIs; old app URLs redirect home and old API URLs return `410`. Nyxify now serves Meting cover art through a bounded fixed-target same-origin route, uses a cleaner circular play control, and consistently uses the existing Spotify-style icon for its shortcut, favicon, and in-app brand mark. These changes have not been committed, pushed, or deployed. The protected untracked local work directories remain present and untouched.
