...
- Latest application release: commit `8076435` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-14. NyxTube remains retired. Nyxify resolves and briefly caches fixed Meting media redirects, prefers the reachable `m801.music.126.net` audio edge with a bounded `m804`/provider-host fallback, keeps a 32 MB LRU of validated cover bytes, and prewarms the seven discovery covers plus bounded 512 KB leading audio ranges after startup. Generated mix tiles now load the real artwork belonging to each mix query's leading track. This removes the long wait caused by OVH timing out against `m701.music.126.net`; the verified production discovery track returned its first 256 KB in 0.27 seconds and the full cached 512 KB prefix in 0.22 seconds.
- Working tree after the release contains uncommitted private-browser-tab and Nyx AI candidates: per-tab proxy cookie isolation/cleanup, precise-geolocation denial, an Aether-backed Movies app with a theatre-mask icon, API-key-aware Nocturne model validation, and local KaTeX/Markdown table rendering for AI responses. The browser privacy work has passed syntax, build, deployment, UV isolation/cleanup/geolocation, and Scramjet cookie-jar isolation checks; the AI work has passed syntax, local catalog/API, KaTeX/TSV rendering, and narrow-layout checks. None of these changes has been committed, pushed, or deployed. The protected untracked local work directories remain present and untouched.
