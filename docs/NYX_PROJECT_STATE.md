...
- Latest application release: commit `8d957dc` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-13. NyxCloud remains retired, while Nyxify now has a music-focused discovery home with real popular-track and artist shelves, generated song mixes, a compact library, and responsive desktop/ChromeOS/mobile layouts. Nyxify prefers the official SoundCloud API only when both server credentials are configured and otherwise retains the public Meting-compatible catalog as an automatic fallback; fixed-target same-origin relays serve either provider without exposing credentials. NyxTube now opens on the official YouTube `videos.list` popular feed, retains quota-cached search, and uses a responsive video-first layout with the standard/private official embed toggle and source link. Production verification confirmed real Nyxify discovery and Adele playback at ready state 4, 18 real popular NyxTube videos, the ChromeOS standard embed path, no horizontal overflow or browser errors, healthy custom domains, and active Nyx/Caddy/coturn services.
...
- NyxTube feed/search/playback through the official YouTube API and embedded player, plus Nyxify search/playback through optional official SoundCloud and the public Meting-compatible fallback
