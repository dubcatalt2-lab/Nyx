...
- Latest application release: commit `96c327e` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-21. Nyx AI exposes the provider-verified Claude Sonnet 4.5 model alongside DeepSeek V4 Flash and retains a user's Sonnet selection after reload. The release baseline retains per-tab in-site browser history, the wide homepage connection panel, Link Checker blocker details and rate-limit handling, current Chat commands and emoji catalog, theme-aware constellations, verified vision upload path, local single-file launcher, and OVH Link Generator fallback.
