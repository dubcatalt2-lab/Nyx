...
- Games-only single-file export/download behavior; the Settings action fetches a self-contained portable arcade into a completed browser Blob before saving `Nyx-Games.html`, and `/download/nyx-singlefile.html` remains a direct attachment route with the same filename. The downloaded file contains Snake, Pong, Breakout, and Moon Match entirely inline, makes no request to or iframe of the Nyx production origin, contains no ads, and works from its local `file:///.../Nyx-Games.html` address without an internet connection. It intentionally does not bundle the full Pirate Cove catalog or Nyx services.
