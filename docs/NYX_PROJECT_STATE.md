...
- Latest application release: commit `e107920` on `agent/pirate-cove`, deployed to OVH on 2026-08-12. Nyx Chat groups the member sidebar by canonical role rank; uses distinct readable palettes for special-role usernames; and animates each role's restrained hover-only color wave over 4.8 seconds. Chat keeps its notification for every incoming channel message and DM, uses louder notification tones, and gives direct username and `@everyone` mentions a stronger three-note ping in both embedded and standalone Chat. Mention parsing recognizes normal punctuation boundaries while preserving the Moderator-or-higher `@everyone` restriction. Owner and Co-owner retain permanent Caffeine with unlimited distinct gifts. It preserves search-history recording, the scoped TikTok Ultraviolet compatibility path, persistent timed Chat mutes, Founder Owner moderation hierarchy overrides, Firebase-authenticated Socket.IO delivery, voice signaling and coturn, normal subscription and recipient Caffeine rules, private DMs, attachments, reactions, mentions, role-restricted channels, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing.
- Unreleased working tree: the Owner Dashboard's selected-account Search history viewer now offers a confirmed **Clear history** action when the current staff member may clear that target. The server permanently removes only that account's retained searches, records the count in the audit log, and enforces role hierarchy: staff may clear their own or lower-ranked accounts, while only the Founder Owner may clear Owner history. The protected untracked local work directories remain present and untouched.
