...
- Latest application release: commit `06316e5` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-21. Browser tabs reject and recover exact external `/unidentified` transitions, oversized AI paste becomes a downloadable text attachment, the guarded homepage health probe updates every second while visible, and the former remote single-file launcher is now the self-contained games-only `Nyx-Games.html` arcade. Nyx AI retains the provider-verified Claude Sonnet 4.5 model alongside DeepSeek V4 Flash and preserves a user's Sonnet selection after reload. The release baseline also retains per-tab in-site browser history, the wide homepage connection panel, Link Checker blocker details and rate-limit handling, current Chat commands and emoji catalog, theme-aware constellations, verified vision upload path, and OVH Link Generator fallback.
