...
- Latest application release: commit `409dda4` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-12. The Founder Owner can create, edit, place, color, permission, assign, and remove custom roles through Owner Dashboard. Custom roles are enforced server-side, retain a built-in hierarchy placement, and appear consistently in Owner Dashboard and Chat. Chat adds permission-aware `/roleadd`, `/roleremove`, `/roles`, `/userinfo`, `/avatar`, `/channelinfo`, `/poll`, and `/timestamp` commands; common emoji shortcodes such as `:smiley:`; and Minecraft-style `&` formatting for both display names and message bodies without changing canonical account handles. The formatter parses the full `&0`-`&f`, `&k`, `&l`, `&m`, `&n`, `&o`, and `&r` palette, preserves clickable mentions, and uses a cache-versioned client asset. Profiles add Starlight Ribbon, Cherry Bloom, Ocean Caustics, Crystal Crown, Lunar Halo, and Rose Vines cosmetics. The release preserves Scramjet recovery, the selected-account Search history viewer and clear action, ranked Chat members, role palettes and hover wave, notification sounds, Firebase-authenticated Socket.IO delivery, voice signaling and coturn, Caffeine, DMs, attachments, reactions, mentions, role-restricted channels, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing.
- Unreleased working tree: Minecraft-formatted display-name segments now preserve Chat's hover wave and the saved profile gradient/neon/toon presentation using each `&` color as the segment base. `&k` performs real bounded glyph obfuscation in Chat and profile surfaces instead of a simple CSS shake, with a static obfuscated fallback under reduced motion. The full code palette and reset/accumulation semantics are under release validation. The protected untracked local work directories remain present and untouched.
