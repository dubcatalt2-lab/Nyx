...
- Latest application release: commit `1ea644e` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-15. The browser tab now presents StudyHub from first paint through the completed startup transition, using the StudyHub title and dedicated `S.` favicon. A one-time client migration replaces the former default Nyx tab identity while preserving user-selected Google, ClassLink, and custom tab cloaks. The earlier Nyxify feed removal, rose-profile, AI display-name, private-tab, Movies, AI model-discovery, KaTeX, and Markdown/TSV work remains part of the release baseline. The live key currently authorizes 23 models; Claude Opus 5 is not exposed because Nocturne rejects the known Claude Opus IDs for this key.
