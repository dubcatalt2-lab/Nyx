...
- Latest application release: commit `fe560e8` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-13. Owner Dashboard custom-role editors now use compact, theme-isolated controls with a synchronized native color picker, and Tide privacy recognizes the live persisted `tide-stressed` role ID as well as the legacy `tide` ID. Owner and the assigned Tide user see Tide; every other viewer receives the Moderator projection and cannot list the private role.
