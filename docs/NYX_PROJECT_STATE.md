...
- The redesigned homepage uses a sparse, reduced-motion-aware constellation field on a jet-black canvas, with the same compact hero geometry across themes. Theme choices are an accent layer rather than a global tint: normal text, navigation icons, fields, and panels stay in a neutral white/gray/black hierarchy, while the selected color is reserved for the particle wordmark, dots, focus rings, active/selected states, and restrained hover feedback. The same balance applies to the generated Apps and Settings workspaces. The particle wordmark owns the visible **NYX** title after its canvas initializes; theme text rules must not expose the solid fallback letters underneath it. **Home design** in Settings can restore the maintained original homepage, which disables redesign-only styles and behavior rather than leaving parts of the new shell behind. The browser tab drawer must remain reachable above Settings so that Settings never traps navigation. The redesigned homepage and Apps workspace must fit without horizontal or vertical clipping down to a 320-pixel-wide viewport. The Apps shortcut opens the complete Apps tab directly; the retired hidden app-launcher drawer and its duplicate catalog markup must not be restored.
- The redesigned homepage background follows the Fern star-theme composition: a charcoal-black surface, a denser field of tiny stars, and five to seven larger edge-positioned constellation webs built from nearest-neighbor links on normal desktop viewports. Narrow screens and medium performance mode use fewer groups to preserve space and rendering headroom. The stars and connecting lines derive their visible color from the active built-in or custom theme and repaint immediately when the theme changes. The lightweight Fern canvas remains the redesigned home's background even when the legacy 3D-background preference is saved. Medium and minimum/Lag Reducer modes keep a lower-density static field visible while disabling its pointer motion and other heavier effects.
