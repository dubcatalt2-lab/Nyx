...
- Unreleased working tree: NyxCloud and its SoundCloud server/configuration surface are removed. Its old app URL redirects to `/`, its former API routes return a JSON `410`, and saved homepage shortcuts are filtered during migration. NyxTube and Nyxify now use a shared Echo-inspired visual language—near-black dotted canvas, slim icon rail, centered search-first hero, compact action pills, large rounded media tiles, and a responsive mobile dock—without copying Echo artwork or branding. NyxTube remains a video-first YouTube search/player and now defaults ChromeOS to YouTube's standard official embed host with a saved privacy-player compatibility toggle. Nyxify retains its additional queue, liked/recent, playlist, shuffle, repeat, seek, and volume features, and its playback now traverses a fixed-target same-origin `/api/music/stream/:trackId` byte-range relay so managed devices do not need direct access to NetEase media hosts. The canonical homepage also carries a complete StudyHub search title/description plus matching Open Graph and Twitter metadata. These changes are local, validated at desktop/mobile sizes and with advancing same-origin audio playback, and not committed, pushed, or deployed. The protected untracked local work directories remain present and untouched.
...
- `apps/nyxtube/`: video-first YouTube presentation; `apps/media-browser.*` provides its same-origin status/search/player behavior
...
- NyxTube search/playback through the official YouTube API and privacy-enhanced embedded player, plus Nyxify search/playback through the public Meting-compatible service
...
- NyxTube calls Nyx's same-origin `/api/nyxtube/*` bridge, searches only embeddable videos through YouTube Data API v3, and plays them with an official YouTube IFrame player. Privacy-enhanced playback remains the normal default; ChromeOS defaults to the standard official embed host for managed-device compatibility, and the player header exposes a saved toggle between the two official hosts. Its server credential remains only in `/etc/nyx/nyx.env`; callers are rate-limited and identical searches are cached briefly. NyxTube must not extract, cache, or retransmit raw YouTube media URLs or expose the Google credential.
- Keep NyxTube video-first and Nyxify music-first while preserving their shared Echo-inspired navigation, search, tile, and mobile-control language. If YouTube is not configured, preserve NyxTube's full layout and show its compact in-page setup card plus noninteractive feed preview; do not replace the workspace with a giant error banner or pretend preview content is live.
- Nyxify is a distinct music workspace at `/apps/nyxify/`. Its same-origin `/api/nyxify/*` bridge searches the documented public Meting-compatible endpoint, validates the returned `api.qijieya.cn/meting/` asset URLs, and requires no API key. Selected tracks play through `/api/music/stream/:trackId`, a fixed-target byte-range relay that accepts only numeric NetEase track IDs and trusted Meting/NetEase audio responses; it must never become a caller-supplied URL proxy. Its playback queue, shuffle/repeat state, recent history, liked songs, and playlists are device-local. It must remain branded as Nyxify rather than pretending to be Spotify, and the upstream community service must be treated as an availability risk.
...
- NyxTube search depends on Google project quota. Nyxify depends on the community-operated `api.qijieya.cn` Meting service, which has no guaranteed availability or service-level agreement. The two apps fail independently.
