...
- Latest application release: commit `d97aa2c` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-20. It preserves the browser-link and Chat-interaction release from `fec4740` and fixes the redesign update log so one browser-profile acknowledgement survives the transition between device and signed-in account state. Legacy device/account acknowledgements migrate to the new marker, the marker is written when the dialog is shown, and the redundant one-time footer sentence has been removed. The standalone `nyx.html` redirect remains ready for an optional Google Cloud Storage upload; no Google bucket or Google-hosted link has been provisioned.
