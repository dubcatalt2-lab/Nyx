...
- Latest application release: commit `a7ac020` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-21. Browser tabs now capture ordinary in-frame navigation in their per-tab history, so Back traverses clicked pages within a site before returning to the Nyx home surface. The release baseline retains the wide homepage connection panel, Link Checker blocker details and rate-limit handling, current Chat commands and emoji catalog, theme-aware constellations, verified vision upload path, local single-file launcher, and OVH Link Generator fallback.
