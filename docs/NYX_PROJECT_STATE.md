...
- Latest application release: commit `290a678` on `agent/pirate-cove`, deployed to OVH and production-verified on 2026-08-18. It retains the balanced minimalist homepage/theme system, live Performance dashboard metrics, refreshed Owner Dashboard and Chat presentation, account-action notices, protected profile-media resolution, corrected Chat navigation, server-authorized moderation commands, personal Nyx AI key/preferences handoff, and exact-name search metadata from the prior release. Nyxify no longer renders the catalog chart or makes an automatic discovery request when opened; its same-origin search, playback, likes, history, playlists, queue, fixed player, and responsive library surfaces remain available.
...
- 2026-08-18 Nyxify compact-chart release: OVH fast-forwarded to `341dacb`, rebuilt the VPS bundle, passed deployment and branding checks, restarted Nyx, and validated Caddy and TURN. The apex, `www`, and FreeDNS alias health routes plus the cache-versioned Nyxify page, CSS, and JavaScript returned HTTP 200. Production Chromium rendered the 72-pixel rail, 58-pixel top bar, and 86-pixel fixed player without horizontal overflow or page errors; deterministic desktop and narrow tests also covered chart rows, search, and the library drawer. The configured Meting fallback returned an upstream HTTP 502 during deployment verification, and the client degraded to its empty state without breaking the page; this remains the existing external-provider availability risk documented below.
- 2026-08-18 Nyxify chart-removal release: OVH fast-forwarded to `290a678`, rebuilt the VPS bundle, passed deployment and branding checks, restarted Nyx, and validated Caddy and TURN. The apex, `www`, and FreeDNS alias health routes returned healthy embedded-Wisp and Socket.IO responses, and live cache-versioned Nyxify assets no longer contained the removed chart markup or automatic chart-loading path. Production Chromium confirmed zero chart rows, zero automatic search requests, no console or page errors, and no horizontal overflow on desktop or mobile while retaining manual search.
...
- The redesigned homepage uses a sparse, reduced-motion-aware constellation field on a jet-black canvas, with the same compact hero and launcher geometry across themes. Theme choices are an accent layer rather than a global tint: normal text, navigation icons, fields, and panels stay in a neutral white/gray/black hierarchy, while the selected color is reserved for the particle wordmark, dots, focus rings, active/selected states, and restrained hover feedback. The same balance applies to the generated Apps and Settings workspaces. The particle wordmark owns the visible **NYX** title after its canvas initializes; theme text rules must not expose the solid fallback letters underneath it. **Home design** in Settings can restore the maintained original homepage, which disables redesign-only styles and behavior rather than leaving parts of the new shell behind. The browser tab drawer must remain reachable above Settings so that Settings never traps navigation. The redesigned homepage and Apps launcher must fit without horizontal or vertical clipping down to a 320-pixel-wide viewport.
- The homepage dashboard uses six equal-width, neutral tabs with a restrained themed underline instead of tinting or pill-filling every section. Its Performance page reports a recent `requestAnimationFrame` rate, the browser-exposed JavaScript heap usage and heap limit when available, an actual same-origin `/healthz` HTTP round trip sampled every five seconds while that page is visible, browser-reported logical CPU threads, the current service-worker registration/controller state, and the user's selected proxy engine. Unsupported or absent signals must say so rather than displaying a guessed value; hidden-tab pauses reset the frame sample so a resumed page does not publish a false FPS drop.
