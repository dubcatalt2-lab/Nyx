...
- Latest application release: commit `1c3dac4` on `agent/pirate-cove`, deployed to OVH on 2026-08-11. Nyx Chat groups the member sidebar by canonical role rank; uses distinct readable palettes for special-role usernames (pink Owner, gold Co-owner, red Admin, purple Manager, blue Developer, green Moderator, teal Support, orange Tester, and silver Contributor); and animates each role's restrained color wave only while that username is hovered. Owner and Co-owner retain permanent Caffeine with unlimited distinct gifts. It preserves search-history recording, the scoped TikTok Ultraviolet compatibility path, louder Chat notifications, persistent timed Chat mutes, Founder Owner moderation hierarchy overrides, Firebase-authenticated Socket.IO delivery, voice signaling and coturn, normal subscription and recipient Caffeine rules, private DMs, attachments, reactions, mentions, role-restricted channels, Caddy On-Demand TLS, Link Checker, account-session reliability, and Chromebook workspace sizing.
- Unreleased working tree: the special-role hover wave now travels at half its previous speed. Chat keeps its notification for every incoming channel message and DM, raises the notification volume, and gives direct username and `@everyone` mentions a louder three-note ping in both embedded and standalone Chat. Mention parsing recognizes normal punctuation boundaries while preserving the Moderator-or-higher `@everyone` restriction. The protected untracked local work directories remain present and untouched.
