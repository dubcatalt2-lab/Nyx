import {aiModelRule} from './ai-model-policy.mjs';
import {freeAiModels,isFreeAiModel} from './ai-free-models.mjs';
import { createHash, randomBytes, createHmac, timingSafeEqual } from 'node:crypto';
import { aiWebBudget } from './ai-web.mjs';
...
export function aiModelAllowed(model, actor={}) {
  // This role restriction must also hold for explicit per-user Allow rules.
  if(['openai/gpt-6-sol','openai/gpt-5.6-sol-pro'].includes(model)&&actor.owner!==true)return false;
  if(model==='anthropic/claude-opus-5.5'&&!actor.owner&&!actor.premium)return false;
  const rule=aiModelRule(actor,model);
  if(rule?.access==='deny')return false;
  if(rule?.access==='allow')return true;
  return actor.owner===true||actor.premium===true||isPublicAiModel(model);
}

export function aiMonthlyModelCap(model,actor={}) {
  return actor.owner?null:actor.premium?50000:10000;
}
const isClaude=model=>model==='anthropic/claude-opus-5.5';
export function aiOwnerClaudeUsage(ledger={},time=Date.now()) {
  const previous=ledger.ownerClaudePool;
  if(previous&&Number.isFinite(previous.start)&&previous.start+4*DAY>time)return {...previous,resetAt:previous.start+4*DAY,used:count(previous.used)};
  const legacy=!previous&&ledger.month===new Date(time).toISOString().slice(0,7)?Object.entries(ledger.tokens||{}).reduce((sum,[model,tokens])=>sum+(isClaude(model)?count(tokens):0),0):0;
  return {start:time,resetAt:time+4*DAY,used:legacy};
}
export function aiTokenPoolUsage(ledger={},premium={},actor={},time=Date.now()) {
  const month=new Date(time).toISOString().slice(0,7);
  const previous=ledger.pool,limit=actor.premium?50000:10000;
  const period='four-days';
  // Shorten existing windows from their original start without refilling usage.
  const previousStart=Number.isFinite(previous?.start)?previous.start:previous?.resetAt-14*DAY;
  const previousReset=Math.min(previous?.resetAt,previousStart+4*DAY);
  if(previous&&Number.isFinite(previousReset)&&previousReset>time)return {...previous,resetAt:previousReset,period,limit,used:count(previous.used),images:count(previous.images),remaining:Math.max(0,limit-count(previous.used))};
  const resetAt=time+4*DAY;
  const legacy=!previous&&premium.month===month?count(premium.tokens):0;
  const used=!previous&&ledger.month===month?Object.entries(ledger.tokens||{}).reduce((total,[model,value])=>model==='openai/gpt-5.6-sol-pro'?total:Math.min(Number.MAX_SAFE_INTEGER,total+count(value)),legacy):legacy;
...
const additionalPrices={
  ...
  'shared:anthropic/claude-opus-5.5':{inputPerMillion:4,outputPerMillion:20,imageTokens:8192},
  'shared:openai/gpt-6-sol':{inputPerMillion:2,outputPerMillion:10,imageTokens:8192},
  'shared:google/gemini-2.5-flash-image':{inputPerMillion:0.30,outputPerMillion:30,imageTokens:8192},
  'shared:inception/mercury-2.5':{inputPerMillion:0.20,outputPerMillion:0.75},
  'shared:qwen/qwen3.7-flash':{inputPerMillion:0.03,outputPerMillion:0.13,imageTokens:8192},
  'shared:deepseek/deepseek-v4.1-flash':{inputPerMillion:0.30,outputPerMillion:1.20,imageTokens:8192},
  'shared:openai/gpt-5.6-sol-pro':{inputPerMillion:4,outputPerMillion:20,imageTokens:8192}
};

export function premiumModelLimits(value={}) {
  return {luna:int(value?.luna,25000,0,10000000),gemini:int(value?.gemini,50000,0,10000000)};
}
export function premiumModelUsage(data={},month) {
  if(data.month!==month)return {luna:0,gemini:0,legacy:0};
  const legacy=count(data.legacyTokens??(data.modelTokens?0:data.tokens));
  return {luna:count(data.modelTokens?.luna)+legacy,gemini:count(data.modelTokens?.gemini)+legacy,legacy};
}
const premiumModelKey=model=>model==='openai/gpt-5.6-luna'?'luna':model==='google/gemini-2.5-flash-lite'?'gemini':null;
...
export function aiAllowanceConfig(env={}) {
  ...
  prices={...additionalPrices,...prices,...Object.fromEntries(freeAiModels.map(model=>['shared:'+model,{inputPerMillion:0,outputPerMillion:0}]))};
  ...
  return {dailyRequests:int(env.NYX_AI_DAILY_REQUEST_BUDGET,1000),dailyUsd,
    ...
    priorityMinute:int(env.NYX_AI_PRIORITY_REQUESTS_PER_MINUTE,12,1,60),
    globalConcurrent:int(env.NYX_AI_CONCURRENT_GLOBAL,30,1,32),
...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  function tier(actor,usage) {
    ...
    if(isFreeAiModel(actor.freeModel))return 'established';
    if(actor.owner)return 'reserve';
    if(actor.premium||actor.coOwner||actor.trusted||aiModelRule(actor,actor.requestedModel)?.access==='allow'||config.trusted.has(actor.uid))return 'established';
  ...
  const references=actor=>({account:ref(`${actor.apiVerified?'api-account':'account'}-${hash(actor.uid)}`),premium:ref(`premium-${hash(actor.uid)}`),models:ref(`models-${hash(actor.uid)}`),global:ref('global')});
  async function begin(actor) {
    ...
    const accepted=await db.runTransaction(async tx=>{
      const [a,g,p,m,custom]=await Promise.all([tx.get(refs.account),tx.get(refs.global),tx.get(refs.premium),tx.get(refs.models),modelRef?tx.get(modelRef):null]);
      const previous=custom?.data()||{};
      const start=Number.isFinite(previous.start)&&now()<previous.start+(rule?.periodDays||4)*DAY?previous.start:now();
      const modelUsed=start===previous.start?count(previous.used):0;
      if(rule?.messages!==null&&rule?.messages!==undefined&&modelUsed>=rule.messages)throw failure(`Your message limit for this model is reached. It resets on ${new Date(start+rule.periodDays*DAY).toISOString()}.`,429,Math.max(1,Math.ceil((start+rule.periodDays*DAY-now())/1000)));
      const raw=a.data()||{},account=state(raw),global=state(g.data()),selected=tier(actor,raw);
      const priority=actor.owner===true||actor.coOwner===true||actor.premium===true;
      const slots=slotValues(raw),globalSlots=slotValues(g.data()),minute=recent(raw.minute);
      ...
      if(!priority&&globalSlots.filter(s=>!s.priority&&s.tier!=='reserve').length>=Math.max(1,config.globalConcurrent-1))throw failure('Nyx AI is busy. Please wait a moment.',429,5);
      ...
      const minuteLimit=actor.apiVerified?int(actor.apiMinuteRequests,4,1,30):priority?Math.max(config.minute,config.priorityMinute):config.minute;
      if(minute.length>=minuteLimit)throw failure('Please wait before sending another AI message.',429,Math.max(1,Math.ceil((Math.min(...minute)+MINUTE-now())/1000)));

      const maximum=actor.apiVerified?int(actor.apiDailyRequests,20,1,1000):actor.premium?1000:Math.min(MAX_MESSAGES,selected==='newcomer'?config.newDaily:config.establishedDaily);
      if(!free&&!actor.owner&&!actor.premium&&!actor.apiVerified&&count(account.requests)>=BASE_MESSAGES&&count(account.tokens)>=BONUS_TOKEN_LIMIT)throw failure('Your 5-message AI allowance is used. Extra messages require a low-token day. Try again after 00:00 UTC.',429,Math.ceil((Date.parse(`${day()}T00:00:00Z`)+DAY-now())/1000));
      if(!free&&!actor.owner&&count(account.requests)>=maximum)throw failure('Your shared AI allowance resets at 00:00 UTC.',429,Math.ceil((Date.parse(`${day()}T00:00:00Z`)+DAY-now())/1000));
      const used=count(global[`${selected}Requests`]),limit=Math.floor(config.dailyRequests*share(selected));
      if(!free&&((!actor.owner&&used>=limit)||['newcomerRequests','establishedRequests','reserveRequests'].reduce((sum,key)=>sum+count(global[key]),0)>=config.dailyRequests))throw failure('This shared AI allowance has been used for today. Please try again after 00:00 UTC.',429,Math.ceil((Date.parse(`${day()}T00:00:00Z`)+DAY-now())/1000));

      const paced=!free&&selected==='newcomer'?bucket(global.newRequestBucket,limit,1):null;
      const slot={id,tier:selected,priority,until:now()+180000};
      tx.set(refs.account,{...raw,...account,requests:count(account.requests)+(free?0:1),minute:[...minute,now()],slots:[slot],activeDays:days(raw),updatedAt:now()});
      tx.set(refs.global,{...global,[`${selected}Requests`]:used+(free?0:1),slots:[...globalSlots,slot],...(paced?{newRequestBucket:paced}:{})});
      if(!actor.owner){
        const pool=aiTokenPoolUsage(m.data(),p.data(),actor,now());
        tx.set(refs.models,{...m.data(),pool:{start:pool.start,resetAt:pool.resetAt,period:pool.period,used:pool.used,images:pool.images,luna6Tokens:count(pool.luna6Tokens),opus55Tokens:count(pool.opus55Tokens)}});
      }
      if(modelRef)tx.set(modelRef,{start,used:modelUsed+1});
      return {tier:selected,messageNumber:count(account.requests)+1,modelStart:start};
    });
    return {id,actor,refs,free,modelRef,modelStart:accepted.modelStart,tier:accepted.tier,messageNumber:accepted.messageNumber,day:day(),calls:0,closed:false};
  }
  function estimate(session,provider,payload) {
    ...
    const web=aiWebBudget(payload);
    if(web===false||payload.functions?.length||Number(payload.n||1)!==1)throw failure('This shared AI request type is unavailable.',400);
    ...
    const passes=web?.passes||1,webInput=web?.input||0,totalOutput=capped*passes;
    if(!config.dailyUsd)return {reserved:0,tokens:bytes*passes+webInput+totalOutput,price:null,provider,model};
    ...
    const input=(bytes+(images?images*Number(price.imageTokens):0))*passes+webInput;
    ...
    const reserved=Math.ceil(input*inputRate+totalOutput*outputRate+(fixed*passes+(web?.feeUsd||0))*USD);
    if(!Number.isSafeInteger(reserved))throw failure('This model needs a valid shared AI price configuration.',503);
    return {reserved,tokens:input+totalOutput,price:{inputRate,outputRate,fixed:fixed*passes,webFee:web?.feeUsd||0},provider,model};
  }
  async function reserve(session,provider,payload) {
    if(!aiModelAllowed(payload.model,session.actor))throw failure('This AI model is not enabled for your account.',403);
    if(session.actor.modelRules?.length&&session.actor.requestedModel&&payload.model!==session.actor.requestedModel)throw failure('This request must use the selected model.',403);
    if(session.closed||++session.calls>4)throw failure('This AI request has reached its processing limit.',429);
    if(session.free){
      if(provider!=='shared'||payload.model!==session.actor.freeModel||!isFreeAiModel(payload.model)||payload.tools?.length||payload.plugins?.length||payload.modalities?.includes('image'))throw failure('Free sessions accept only the selected free text model.',400);
    }
    const cost=estimate(session,provider,payload),id=randomBytes(12).toString('hex');
    // Free provider routing still costs zero dollars, but shares the account token pool.
    if(session.free){cost.reserved=0;cost.price={inputRate:0,outputRate:0,fixed:0};}
    // Bound OpenRouter routing to the same rates used by the spending reservation.
    if(provider==='shared'&&additionalPrices[`shared:${payload.model}`]&&cost.price)payload.provider={...payload.provider,max_price:{prompt:cost.price.inputRate,completion:cost.price.outputRate}};
    const modelCap=aiMonthlyModelCap(payload.model,session.actor);
    const ownerClaude=Boolean(session.actor.owner&&isClaude(payload.model));
    const imageRequest=!session.actor.owner&&payload.model==='google/gemini-2.5-flash-image'&&payload.modalities?.includes('image');
    const trackPremium=Boolean(!session.free&&session.actor.premium&&!session.actor.owner&&!additionalPrices[`shared:${payload.model}`]);
    await db.runTransaction(async tx=>{
      const [a,g,p,m]=await Promise.all([tx.get(session.refs.account),tx.get(session.refs.global),tx.get(session.refs.premium),tx.get(session.refs.models)]);
      ...
      if(ownerClaude&&claudePool.used+cost.tokens>50000)throw failure(`This request exceeds your 50,000-token Claude Opus 5.5 allowance. It resets on ${new Date(claudePool.resetAt).toISOString().slice(0,10)} (UTC).`,429,Math.max(1,Math.ceil((claudePool.resetAt-now())/1000)));
      if(!session.actor.owner&&payload.model==='anthropic/claude-opus-5.5'&&count(tokenPool.opus55Tokens)+cost.tokens>5000)throw failure(`This request exceeds your remaining Claude Opus 5.5 allowance of 5,000 tokens within your shared pool. It resets on ${new Date(tokenPool.resetAt).toISOString().slice(0,10)} (UTC).`,429,Math.max(1,Math.ceil((tokenPool.resetAt-now())/1000)));
      ...
      const used=session.actor.owner?count(modelLedger[payload.model]):tokenPool.used;
      if(modelCap!==null&&used+cost.tokens>modelCap)throw failure(session.actor.owner?'This request exceeds your remaining monthly Sol Pro tokens.':`This request exceeds your shared ${tokenPool.limit.toLocaleString('en-US')}-token pool. It resets on ${new Date(tokenPool.resetAt).toISOString().slice(0,10)} (UTC).`,429,3600);
      if(!session.free&&!session.actor.owner&&!session.actor.premium&&!session.actor.apiVerified&&session.messageNumber>BASE_MESSAGES&&count(account.tokens)+cost.tokens>BONUS_TOKEN_LIMIT)throw failure('This extra message would exceed your daily token allowance. Shorten the conversation or try again after 00:00 UTC.',429,3600);
      ...
      if(!session.free&&config.dailyUsd&&((!session.actor.owner&&count(global[moneyKey])+cost.reserved>pool)||['newcomerMoney','establishedMoney','reserveMoney'].reduce((sum,key)=>sum+count(global[key]),0)+cost.reserved>config.dailyUsd||(!session.actor.owner&&count(account.money)+cost.reserved>personal)||spentMonth+cost.reserved>config.monthlyUsd))throw failure('The shared AI spending allowance has been reached. Please try again later.',429,3600);
      ...
      tx.set(session.refs.account,{...raw,...account,money:count(account.money)+cost.reserved,tokens:count(account.tokens)+cost.tokens,receipts:[...receipts,{id,reserved:cost.reserved,tokens:cost.tokens,day:day(),premium:trackPremium,modelKey,imageRequest,ownerClaudeStart:ownerClaude?claudePool.start:null,quotaModel:modelCap!==null||ownerClaude?payload.model:null,poolStart:!session.actor.owner?tokenPool.start:null,tier:session.tier,until:now()+180000}]});
      if(modelCap!==null||ownerClaude)tx.set(session.refs.models,{...m.data(),month,...(ownerClaude?{ownerClaudePool:{...claudePool,used:claudePool.used+cost.tokens}}:{}),...(!session.actor.owner?{pool:{start:tokenPool.start,resetAt:tokenPool.resetAt,period:tokenPool.period,used:tokenPool.used+cost.tokens,images:tokenPool.images+(imageRequest?1:0),luna6Tokens:count(tokenPool.luna6Tokens)+(isLuna6(payload.model)?cost.tokens:0),opus55Tokens:count(tokenPool.opus55Tokens)+(payload.model==='anthropic/claude-opus-5.5'?cost.tokens:0)}}:{}),tokens:{...modelLedger,[payload.model]:count(modelLedger[payload.model])+cost.tokens}});
    ...
    return {...cost,free:session.free,id,day:day(),session};
  }
  async function settle(reservation,usage,notSent=false,imageCount=null) {
    ...
    if(reservation.price&&usage&&Number.isFinite(usage.input)&&Number.isFinite(usage.output)&&usage.input>=0&&usage.output>=0) {
      ...
      charged=Math.ceil(usage.input*p.inputRate+usage.output*p.outputRate+(p.fixed+(usage.webSearches===0?0:p.webFee||0))*USD);
    ...
    await db.runTransaction(async tx=>{
      ...
      if(receipt.premium&&p.data()?.month===receipt.day.slice(0,7)){
        const previous=p.data(),usage=premiumModelUsage(previous,previous.month),models={luna:usage.luna-usage.legacy,gemini:usage.gemini-usage.legacy};
        let legacy=usage.legacy;
        if(receipt.modelKey)models[receipt.modelKey]=Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,models[receipt.modelKey]+tokenDifference));
        else legacy=Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,legacy+tokenDifference));
        tx.set(session.refs.premium,{tokens:Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,count(previous.tokens)+tokenDifference)),legacyTokens:legacy,modelTokens:models},{merge:true});
      }
  ...
  async function finish(session,success=false) {
    ...
    await db.runTransaction(async tx=>{
      const [a,g,custom]=await Promise.all([tx.get(session.refs.account),tx.get(session.refs.global),session.modelRef?tx.get(session.modelRef):null]);
      if(!success&&custom?.data()?.start===session.modelStart)tx.set(session.modelRef,{...custom.data(),used:Math.max(0,count(custom.data().used)-1)});
