...
export function aiModelAllowed(model, actor={}) {
  ...
  if(model==='openai/gpt-6-sol'&&actor.owner!==true)return false;
  ...
  if(model==='openai/gpt-5.6-sol-pro')return actor.owner===true;
  return model !== 'openai/gpt-5.6-luna' || actor.owner === true || actor.premium === true;
}

export function aiMonthlyModelCap(model,actor={}) {
  if(model==='openai/gpt-5.6-sol-pro'&&actor.owner)return 100000;
...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  function tier(actor,usage) {
    ...
    if(!actor.owner&&!actor.coOwner&&!actor.apiVerified&&!actor.premium&&!isLuna6(actor.requestedModel)&&aiModelRule(actor,actor.requestedModel)?.access!=='allow'&&!(Number.isFinite(actor.createdAt)&&actor.createdAt<AI_JOIN_CUTOFF))throw failure('Shared AI is available to Premium members and accounts created before August 25, 2026 (Pacific time).',403);
  ...
  async function reserve(session,provider,payload) {
    ...
    if(session.free){
      ...
      estimate(session,provider,payload);
      return {free:true,session,reserved:0,tokens:0,price:{inputRate:0,outputRate:0,fixed:0}};
    }
    ...
    const trackPremium=Boolean(session.actor.premium&&!session.actor.owner&&!additionalPrices[`shared:${payload.model}`]);
    await db.runTransaction(async tx=>{
      ...
      if(!session.actor.owner&&!session.actor.premium&&isLuna6(payload.model)&&count(tokenPool.luna6Tokens)+cost.tokens>5000)throw failure(`This request exceeds your remaining GPT-6 Luna allowance of 5,000 tokens within your shared pool. It resets on ${new Date(tokenPool.resetAt).toISOString().slice(0,10)} (UTC).`,429,Math.max(1,Math.ceil((tokenPool.resetAt-now())/1000)));
      ...
      if(!session.actor.owner&&!session.actor.premium&&!session.actor.apiVerified&&session.messageNumber>BASE_MESSAGES&&count(account.tokens)+cost.tokens>BONUS_TOKEN_LIMIT)throw failure('This extra message would exceed your daily token allowance. Shorten the conversation or try again after 00:00 UTC.',429,3600);
      ...
      if(config.dailyUsd&&((!session.actor.owner&&count(global[moneyKey])+cost.reserved>pool)||['newcomerMoney','establishedMoney','reserveMoney'].reduce((sum,key)=>sum+count(global[key]),0)+cost.reserved>config.dailyUsd||(!session.actor.owner&&count(account.money)+cost.reserved>personal)||spentMonth+cost.reserved>config.monthlyUsd))throw failure('The shared AI spending allowance has been reached. Please try again later.',429,3600);
