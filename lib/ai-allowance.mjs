...
export function aiTokenPoolUsage(ledger={},premium={},actor={},time=Date.now()) {
  ...
  if(previous&&Number.isFinite(previous.resetAt)&&previous.resetAt>time)return {...previous,limit,used:count(previous.used),remaining:Math.max(0,limit-count(previous.used))};
  ...
  return {limit,used,remaining:Math.max(0,limit-used),start:time,resetAt,period};
}
...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  async function begin(actor) {
    ...
    const accepted=await db.runTransaction(async tx=>{
      ...
      if(!actor.owner){
        ...
        tx.set(refs.models,{...m.data(),pool:{start:pool.start,resetAt:pool.resetAt,period:pool.period,used:pool.used}});
      }
  ...
  async function reserve(session,provider,payload) {
    ...
    await db.runTransaction(async tx=>{
      ...
      tx.set(session.refs.account,{...raw,...account,money:count(account.money)+cost.reserved,tokens:count(account.tokens)+cost.tokens,receipts:[...receipts,{id,reserved:cost.reserved,tokens:cost.tokens,day:day(),premium:trackPremium,modelKey,quotaModel:modelCap!==null?payload.model:null,poolStart:!session.actor.owner?tokenPool.start:null,tier:session.tier,until:now()+180000}]});
      if(modelCap!==null)tx.set(session.refs.models,{...m.data(),month,...(!session.actor.owner?{pool:{start:tokenPool.start,resetAt:tokenPool.resetAt,period:tokenPool.period,used:tokenPool.used+cost.tokens}}:{}),tokens:{...modelLedger,[payload.model]:count(modelLedger[payload.model])+cost.tokens}});
  ...
  async function settle(reservation,usage,notSent=false) {
    ...
    await db.runTransaction(async tx=>{
      ...
      if(ledger&&receipt.quotaModel){
        ...
        if(receipt.poolStart!==null&&receipt.poolStart===ledger.pool?.start)updated.pool={...ledger.pool,used:Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,count(ledger.pool.used)+tokenDifference))};
