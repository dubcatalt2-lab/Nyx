...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  function tier(actor,usage) {
    ...
    if(!actor.owner&&!actor.apiVerified&&!actor.premium&&!(Number.isFinite(actor.createdAt)&&actor.createdAt<AI_JOIN_CUTOFF))throw failure('Shared AI is available to Premium members and accounts created before August 25, 2026 (Pacific time).',403);
