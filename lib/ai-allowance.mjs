...
export function aiAllowanceConfig(env={}) {
  ...
  prices={...additionalPrices,...prices};
...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  async function begin(actor) {
    ...
    const accepted=await db.runTransaction(async tx=>{
      ...
      if(!actor.owner&&!actor.premium&&!actor.apiVerified&&count(account.requests)>=BASE_MESSAGES&&count(account.tokens)>=BONUS_TOKEN_LIMIT)throw failure('Your 5-message AI allowance is used. Extra messages require a low-token day. Try again after 00:00 UTC.',429,Math.ceil((Date.parse(`${day()}T00:00:00Z`)+DAY-now())/1000));
      if(!actor.owner&&count(account.requests)>=maximum)throw failure('Your shared AI allowance resets at 00:00 UTC.',429,Math.ceil((Date.parse(`${day()}T00:00:00Z`)+DAY-now())/1000));
      const used=count(global[`${selected}Requests`]),limit=Math.floor(config.dailyRequests*share(selected));
      if((!actor.owner&&used>=limit)||['newcomerRequests','establishedRequests','reserveRequests'].reduce((sum,key)=>sum+count(global[key]),0)>=config.dailyRequests)throw failure('This shared AI allowance has been used for today. Please try again after 00:00 UTC.',429,Math.ceil((Date.parse(`${day()}T00:00:00Z`)+DAY-now())/1000));
