...
export function aiTokenPoolUsage(ledger={},premium={},actor={},time=Date.now()) {
  ...
  // Keep a running pool through role changes so upgrades/downgrades cannot refill it.
  if(previous&&Number.isFinite(previous.resetAt)&&previous.resetAt>time)return {...previous,limit,used:count(previous.used),images:count(previous.images),remaining:Math.max(0,limit-count(previous.used))};
  const period='fortnight';
  const resetAt=time+14*DAY;
