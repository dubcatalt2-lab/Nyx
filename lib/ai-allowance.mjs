...
export function aiMonthlyModelCap(model,actor={}) {
  if(model==='openai/gpt-5.6-sol-pro')return 100000;
...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  function tier(actor,usage) {
    ...
    if(!actor.owner&&!actor.coOwner&&!actor.apiVerified&&!actor.premium&&!(Number.isFinite(actor.createdAt)&&actor.createdAt<AI_JOIN_CUTOFF))throw failure('Shared AI is available to Premium members and accounts created before August 25, 2026 (Pacific time).',403);
    if(actor.owner)return 'reserve';
    if(actor.premium||actor.coOwner||actor.trusted||config.trusted.has(actor.uid))return 'established';
  ...
  async function begin(actor) {
    ...
    const accepted=await db.runTransaction(async tx=>{
      const [a,g,p,m]=await Promise.all([tx.get(refs.account),tx.get(refs.global),tx.get(refs.premium),tx.get(refs.models)]);
    ...
    return {id,actor,refs,free,tier:accepted.tier,messageNumber:accepted.messageNumber,day:day(),calls:0,closed:false};
  }
  ...
  async function reserve(session,provider,payload) {
    if(!aiModelAllowed(payload.model,session.actor))throw failure(payload.model==='openai/gpt-5.6-sol-pro'?'GPT-5.6 Sol Pro is available to the owner only.':'GPT-5.6 Luna is available to Premium members and the owner only.',403);
