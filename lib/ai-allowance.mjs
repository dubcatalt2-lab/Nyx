...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  async function reserve(session,provider,payload) {
    ...
    await db.runTransaction(async tx=>{
      ...
      const premiumLimit=int(session.actor.monthlyTokenLimit,50000,0,10000000);
      if(!session.actor.owner&&session.actor.premium&&payload.model==='openai/gpt-5.6-luna'&&premiumUsed+cost.tokens>premiumLimit)throw failure('Luna is unavailable: your monthly token allowance is used or this request exceeds the remaining tokens. Choose Gemini or ask the owner to adjust your limit.',429,3600);
      ...
      tx.set(session.refs.account,{...raw,...account,money:count(account.money)+cost.reserved,tokens:count(account.tokens)+cost.tokens,receipts:[...receipts,{id,reserved:cost.reserved,tokens:cost.tokens,day:day(),premium:Boolean(session.actor.premium),tier:session.tier,until:now()+180000}]});
      if(session.actor.premium)tx.set(session.refs.premium,{month,tokens:Math.min(Number.MAX_SAFE_INTEGER,premiumUsed+cost.tokens)},{merge:true});
  ...
  async function settle(reservation,usage,notSent=false) {
    ...
    await db.runTransaction(async tx=>{
      ...
      if(receipt.premium&&p.data()?.month===receipt.day.slice(0,7))tx.set(session.refs.premium,{tokens:Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,count(p.data().tokens)+tokenDifference))},{merge:true});
