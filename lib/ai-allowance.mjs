...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  const references=actor=>({account:ref(`${actor.apiVerified?'api-account':'account'}-${hash(actor.uid)}`),premium:ref(`premium-${hash(actor.uid)}`),global:ref('global')});
  async function begin(actor) {
    ...
    const accepted=await db.runTransaction(async tx=>{
      const [a,g]=await Promise.all([tx.get(refs.account),tx.get(refs.global)]);
  ...
  async function reserve(session,provider,payload) {
    if(!aiModelAllowed(payload.model,session.actor))throw failure('GPT-5.6 Luna is available to Premium members and the owner only.',403);
    ...
    await db.runTransaction(async tx=>{
      const [a,g,p]=await Promise.all([tx.get(session.refs.account),tx.get(session.refs.global),tx.get(session.refs.premium)]);
      ...
      const modelKey=premiumModelKey(payload.model),modelUsage=premiumModelUsage(premium,month),limits=premiumModelLimits(session.actor.monthlyModelLimits);
      if(!session.actor.owner&&session.actor.premium&&modelKey&&modelUsage[modelKey]+cost.tokens>limits[modelKey])throw failure(`${modelKey==='luna'?'Luna':'Gemini'} is unavailable: this request exceeds your remaining monthly tokens for this model. Choose another model or ask the owner to adjust your limit.`,429,3600);
      ...
      tx.set(session.refs.account,{...raw,...account,money:count(account.money)+cost.reserved,tokens:count(account.tokens)+cost.tokens,receipts:[...receipts,{id,reserved:cost.reserved,tokens:cost.tokens,day:day(),premium:Boolean(session.actor.premium&&!session.actor.owner),modelKey,tier:session.tier,until:now()+180000}]});
      if(session.actor.premium&&!session.actor.owner)tx.set(session.refs.premium,{month,tokens:Math.min(Number.MAX_SAFE_INTEGER,premiumUsed+cost.tokens),legacyTokens:modelUsage.legacy,modelTokens:{luna:modelUsage.luna-modelUsage.legacy+(modelKey==='luna'?cost.tokens:0),gemini:modelUsage.gemini-modelUsage.legacy+(modelKey==='gemini'?cost.tokens:0)}},{merge:true});
