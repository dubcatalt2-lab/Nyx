...
export function aiTokenPoolUsage(ledger={},premium={},actor={},time=Date.now()) {
  ...
  return {limit,used,images:0,remaining:Math.max(0,limit-used),start:time,resetAt,period};
}
...
export function createAiAllowance({db,config,now=Date.now}) {
  ...
  function tier(actor,usage) {
    ...
    if(!actor.owner&&!actor.coOwner&&!actor.apiVerified&&!actor.premium&&aiModelRule(actor,actor.requestedModel)?.access!=='allow'&&!(Number.isFinite(actor.createdAt)&&actor.createdAt<AI_JOIN_CUTOFF))throw failure('Shared AI is available to Premium members and accounts created before August 25, 2026 (Pacific time).',403);
  ...
  async function begin(actor) {
    ...
    const accepted=await db.runTransaction(async tx=>{
      ...
      if(!free&&!actor.owner){
        ...
        tx.set(refs.models,{...m.data(),pool:{start:pool.start,resetAt:pool.resetAt,period:pool.period,used:pool.used,images:pool.images}});
      }
  ...
  async function settle(reservation,usage,notSent=false,imageCount=null) {
    ...
    await db.runTransaction(async tx=>{
      ...
      if(ledger&&receipt.quotaModel){
        ...
        if(receipt.poolStart!==null&&receipt.poolStart===ledger.pool?.start)updated.pool={...ledger.pool,used:Math.min(Number.MAX_SAFE_INTEGER,Math.max(0,count(ledger.pool.used)+tokenDifference)),images:Math.max(0,count(ledger.pool.images)-(receipt.imageRequest&&(notSent||imageCount===0)?1:0))};
