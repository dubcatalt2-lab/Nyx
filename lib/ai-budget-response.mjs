...
export function aiBudgetResponse(response,onComplete,maxBytes=2*1024*1024) {
  let finished=false,total=0,buffer='',usage=null,failed=false,imageCount=null;
  ...
  const done=async complete=>{if(finished)return;finished=true;await onComplete(complete&&response.ok&&!failed?usage:null,Boolean(complete&&response.ok&&!failed&&total),complete?imageCount:null);};
