...
export function installDeveloperApi(app,deps) {
  ...
  async function accountDetails(u){
    ...
    details.monthlyTokenLimit=u.monthlyTokenLimit;return details;
  }
  ...
  app.post('/api/developer/owner/account/:uid',wrap(async(req,res)=>{const u=await owner(req);const target={...u,...await identity(u.firebase,req.params.uid,true)};const limit=req.body?.monthlyTokenLimit;if(limit!==undefined&&!integer(limit,0,10000000))throw fail('Enter a valid monthly token limit.');await u.store.update(req.params.uid,req.body);if(limit!==undefined){await u.firebase.firestore.collection('nyxUserAdministration').doc(target.uid).set({aiMonthlyTokenLimit:limit},{merge:true});target.monthlyTokenLimit=limit;}res.json(await accountDetails(target));}));
