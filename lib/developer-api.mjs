import {premiumModelLimits,premiumModelUsage} from './ai-allowance.mjs';
...
export function createKeyStore(db,now=Date.now,vault=null) {
  ...
  async function issue(uid,device,label,exempt=false) {
    ...
    const encryptedKey=vault?await vault.seal(secret,`${uid}:${id}`):null;
    await db.runTransaction(async tx=>{
      ...
      tx.set(ref(`key-${id}`),{uid,prefix:secret.slice(0,13),label:String(label||'My API key').trim().slice(0,60),createdAt:now(),revoked:false,...(encryptedKey?{encryptedKey}:{})});
    });
    return {key:secret};
  }
  async function revoke(uid) {
    await db.runTransaction(async tx=>{
      ...
      tx.set(k,{revoked:true,encryptedKey:null},{merge:true});
  ...
  async function details(uid) {
    ...
    return {uid,balance:v.balance,grantedTokens:v.grantedTokens??v.balance+(v.usedTokens||0),usedTokens:v.usedTokens||0,
      ...
      key:k?{prefix:k.prefix,label:k.label,createdAt:k.createdAt,revealAvailable:Boolean(k.encryptedKey)}:null};
  }
  async function reveal(uid,actorUid){
    return db.runTransaction(async tx=>{
      const a=await tx.get(account(uid)),id=a.data()?.activeKey;
      if(!id)throw fail('This user has no active key.',404);
      const r=ref(`key-${id}`),record=(await tx.get(r)).data();
      if(!record||record.revoked)throw fail('This API key is revoked.',404);
      if(!record.encryptedKey)throw fail('This older key was stored as a hash and cannot be revealed. Its user can revoke it and create a replacement without losing their balance.',409);
      if(!vault)throw fail('API key reveal storage is unavailable.',503);
      const key=await vault.open(record.encryptedKey,`${uid}:${id}`);
      if(hash(key)!==id)throw fail('This stored key could not be verified.',503);
      tx.set(r,{lastRevealedAt:now(),lastRevealedBy:actorUid},{merge:true});
      return {key};
    });
  }
  ...
  return {issue,revoke,authenticate,details,update,reserve,settle,reveal,ref};
}
...
export function installDeveloperApi(app,deps) {
  ...
  async function identity(firebase,uid,allowRestricted=false) {
    ...
    const admin=a.data()||{};
    return {uid,premium:['premium','trialing'].includes(admin.subscriptionStatus||admin.subscription?.status),monthlyModelLimits:premiumModelLimits(admin.aiMonthlyModelLimits),owner:uid===deps.ownerUid(),verified:Boolean(u.email&&u.emailVerified)};
  }
  async function user(req,mutation=false) {
    ...
    return {firebase,store:createKeyStore(firebase.firestore,Date.now,deps.keyVault),...await identity(firebase,token.uid)};
  }
  ...
  async function accountDetails(u){
    ...
    if(u.owner){details.models=[GEMINI,LUNA];details.unlimited=true;}
      ...
      const usage=premiumModelUsage((await u.firebase.firestore.collection('nyxAiAllowance').doc(`premium-${hash(u.uid)}`).get()).data()||{},new Date().toISOString().slice(0,7));
      details.modelAllowances=Object.fromEntries([['luna',LUNA],['gemini',GEMINI]].map(([name,id])=>[id,{limit:u.monthlyModelLimits[name],used:usage[name],remaining:Math.max(0,u.monthlyModelLimits[name]-usage[name])}]));
      details.balance=Object.values(details.modelAllowances).reduce((n,v)=>n+v.remaining,0);details.grantedTokens=u.monthlyModelLimits.luna+u.monthlyModelLimits.gemini;details.usedTokens=usage.luna+usage.gemini;
      details.models=Object.entries(details.modelAllowances).filter(([,v])=>v.remaining>0).map(([id])=>id);details.legacyTokens=usage.legacy;
    }
  ...
  app.post('/api/developer/owner/account/:uid',wrap(async(req,res)=>{const u=await owner(req);const target={...u,...await identity(u.firebase,req.params.uid,true)};const limits=req.body?.monthlyModelLimits;
    if(req.body?.monthlyTokenLimit!==undefined)throw fail('Refresh the page to set separate Luna and Gemini limits.');
    if(limits!==undefined&&(!target.premium||target.owner))throw fail('Monthly AI allowances are only available to Premium members.',403);
