...
export function createKeyStore(db,now=Date.now) {
  ...
  async function issue(uid,ip,label) {
    if(!ip)throw fail('Your network address could not be verified.',503);
    const secret=`n_api_${randomBytes(32).toString('base64url')}`,id=hash(secret),ipId=`ip-${hash(ip)}`;
    await db.runTransaction(async tx=>{
      const [a,i]=await Promise.all([tx.get(account(uid)),tx.get(ref(ipId))]);
      const data=a.data()||{...defaults(),balance:i.data()?.starterGranted?0:1000,grantedTokens:i.data()?.starterGranted?0:1000};
      if(data.activeKey)throw fail('Revoke your existing key before creating a replacement.',409);
      if(i.data()?.activeKey)throw fail('This IP address already has an active API key.',409);
      ...
      tx.set(ref(ipId),{activeKey:id,starterGranted:true});
      tx.set(ref(`key-${id}`),{uid,ipId,prefix:secret.slice(0,13),label:String(label||'My API key').trim().slice(0,60),createdAt:now(),revoked:false});
    });
    return {key:secret};
  }
  ...
  async function reserve(key,payload) {
    ...
    await db.runTransaction(async tx=>{
      ...
      if(!a.models.includes(payload.model))throw fail('This model is not enabled for your key.',403);
      const minute=(a.minute||[]).filter(t=>t>now()-60000),day=new Date(now()).toISOString().slice(0,10),requests=a.day===day?a.requests:0;
      if(minute.length>=a.minuteRequests||requests>=a.dailyRequests)throw fail('Your API request limit has been reached.',429);
      ...
      const output=Math.min(payload.max_tokens,a.maxOutput,a.balance-input);
      ...
      tx.set(account(key.uid),{...a,balance:a.balance-reserved,day,requests:requests+1,minute:[...minute,now()],hold:{id,until:now()+180000}});
    });
    return {uid:key.uid,id,reserved,model:payload.model,startedAt:now()};
  }
  async function settle(receipt,usage,notSent=false) {
    ...
    await db.runTransaction(async tx=>{
      ...
      tx.set(account(receipt.uid),{balance:v.balance+receipt.reserved-used,hold:null,usedTokens:(v.usedTokens||0)+used,recent:[...(v.recent||[]).slice(-19),{at:now(),model:receipt.model,tokens:used,status:notSent?'not_sent':valid?'completed':'unconfirmed',durationMs:Math.max(0,now()-receipt.startedAt)}]},{merge:true});
    });
  }
...
export function installDeveloperApi(app,deps) {
  ...
  async function identity(firebase,uid) {
    const [u,a]=await Promise.all([firebase.auth.getUser(uid),firebase.firestore.collection('nyxUserAdministration').doc(uid).get()]);
    if(u.disabled||a.data()?.disabled||a.data()?.aiAccess==='restricted')throw fail('This account cannot use the API.',403);
    return {uid,owner:uid===deps.ownerUid(),verified:Boolean(u.email&&u.emailVerified)};
  }
  ...
  const verified=u=>{if(!u.owner&&!u.verified)throw fail('Verify your Nyx email before creating or using an API key.',403);};
  app.get('/api',deps.page);
  app.get('/api/developer/me',wrap(async(req,res)=>{
    const u=await user(req);res.json({...await u.store.details(u.uid),verified:u.verified,owner:u.owner,unlocked:unlocked(req,u),configured:deps.configured()});
  }));
  app.post('/api/developer/keys',wrap(async(req,res)=>{
    const u=await user(req,true);verified(u);
    if(!deps.configured())throw fail('The owner needs to configure OpenRouter first.',503);
    res.json(await u.store.issue(u.uid,deps.clientIp(req),req.body?.label));
  }));
  ...
  app.get('/api/developer/owner/account/:uid',wrap(async(req,res)=>{const u=await owner(req);await identity(u.firebase,req.params.uid);res.json(await u.store.details(req.params.uid));}));
  app.post('/api/developer/owner/account/:uid',wrap(async(req,res)=>{const u=await owner(req);await identity(u.firebase,req.params.uid);await u.store.update(req.params.uid,req.body);res.json(await u.store.details(req.params.uid));}));
  ...
  app.post('/api/v1/ai',wrap(async(req,res)=>{
    ...
    const u=await identity(firebase,key.uid);verified(u);
    const b=req.body||{};
    ...
    const receipt=await store.reserve(key,payload);
