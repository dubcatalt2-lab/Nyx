import {premiumModelLimits,aiTokenPoolUsage} from './ai-allowance.mjs';
...
export function installDeveloperApi(app,deps) {
  ...
  async function accountDetails(u){
    ...
    if(u.owner){details.models=[GEMINI,LUNA];details.unlimited=true;}
      ...
      const col=u.firebase.firestore.collection('nyxAiAllowance');
      const [ledger,previous]=await Promise.all([col.doc(`models-${hash(u.uid)}`).get(),col.doc(`premium-${hash(u.uid)}`).get()]);
      details.tokenPool=aiTokenPoolUsage(ledger.data(),previous.data(),u);
      if(u.premium){details.balance=details.tokenPool.remaining;details.grantedTokens=details.tokenPool.limit;details.usedTokens=details.tokenPool.used;details.models=details.balance?[GEMINI,LUNA]:[];}
      else details.balance=Math.min(details.balance,details.tokenPool.remaining);
    }
