// Exact TMDB season/episode identities, mapped to AnimeX public embeds.
...
export function createAnimeSources({fetchImpl=fetch,now=Date.now}={}){
 ...
 return async function animeEpisodeSources(id,season,episode){
  ...
  if(!hit.value||match.episode>hit.value.total)return [];
  const {slug,total}=hit.value,e=match.episode;
  return [{id:'animex',name:'AnimeX',url:`https://plyr.animex.one/e/${slug}/${e}?lang=sub&autoplay=1&t=0&hasPrev=${e>1?1:0}&hasNext=${e<total?1:0}`}];
 };
}
