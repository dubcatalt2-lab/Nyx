...
export function installMoviePlayback(app,{clientId=req=>req.ip,resolveStream=resolveMovieStream,open=upstream,validateMovie=async()=>{}}={}){
 ...
 function publicProtocol(req){const forwarded=String(req.get('x-forwarded-proto')||'').split(',')[0].trim().toLowerCase();return process.env.NYX_TRUST_PROXY==='true'&&['http','https'].includes(forwarded)?forwarded:req.protocol;}
 function sameSite(req){return req.headers['sec-fetch-site']!=='cross-site'&&(!req.headers.origin||req.headers.origin===`${publicProtocol(req)}://${req.get('host')}`);}
 ...
 app.post('/api/movies/:id/playback',async(req,res)=>{
  ...
  if(!viewer(req))res.cookie('nyx_movie_viewer',owner,{httpOnly:true,sameSite:'strict',secure:publicProtocol(req)==='https',path:'/api/movies',maxAge:86400000});
