import {createAnimeSources} from './movie-anime-sources.mjs';
import {getMovieMappings} from './movie-mappings.mjs';
import {movieEpisodes} from './movie-episodes.mjs';
import {installPublicMovieRoutes} from './movie-public-api.mjs';
...
export function createMovieCatalog({token=()=>process.env.TMDB_TOKEN,fetchImpl=fetch,now=Date.now,mappings=getMovieMappings}={}){
  ...
  const animeEpisodeSources=createAnimeSources({fetchImpl,now});
  ...
  return {
    ...
    async episode(id,season,episode){
      ...
      let animeSources=await animeEpisodeSources(id,sourceSeason,sourceEpisode);
      if(!animeSources.length&&entry.id&&(sourceSeason!==season||sourceEpisode!==episode))animeSources=await animeEpisodeSources(id,season,episode);
      return {id:`${id}/${season}/${episode}`,kind:'episode',tmdbSeriesId:Number(id),season,episode,sourceSeason,sourceEpisode,title:data.title,episodeLabel:`Season ${season} · Episode ${episode} · ${entry.name}`,sources:[{id:'rive',name:'Rive',url:`https://watch.rivestream.app/embed?type=tv&id=${id}&season=${sourceSeason}&episode=${sourceEpisode}`},{id:'nhd',name:'NHD',url:`https://nhdapi.com/tv/${id}/${sourceSeason}/${sourceEpisode}`},...animeSources,{id:'framextv',name:'FrameXTV',url:`https://framextv.tech/embed/${id}/${sourceSeason}/${sourceEpisode}`}]};
    },
    async details(id){
...
export function installMovieApi(app,{catalog=createMovieCatalog(),clientId=req=>req.ip,now=Date.now}={}){
  ...
  installPublicMovieRoutes(app,{catalog,route});
