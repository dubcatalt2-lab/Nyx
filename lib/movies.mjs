...
export function createMovieCatalog({token=()=>process.env.TMDB_TOKEN,fetchImpl=fetch,now=Date.now,mappings=getMovieMappings}={}){
  ...
  return {
    ...
    async episode(id,season,episode){
      ...
      return {id:`${id}/${season}/${episode}`,kind:'episode',tmdbSeriesId:Number(id),season,episode,title:data.title,episodeLabel:`Season ${season} · Episode ${episode} · ${entry.name}`,sources:[{id:'nhd',name:'NHD',url:`https://nhdapi.com/tv/${id}/${sourceSeason}/${sourceEpisode}`},{id:'screenscape',name:'ScreenScape',url:`https://screenscape.me/embed?tmdb=${id}&type=tv&s=${sourceSeason}&e=${sourceEpisode}&lan=eng`},{id:'vidcore',name:'VidCore',url:`https://vidcore.org/embed/tv/${id}/${sourceSeason}/${sourceEpisode}?theme=eeeeee`}]};
    },
    async details(id){
