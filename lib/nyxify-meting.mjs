...
import { createMediaFetch } from './nyxify-media-fetch.mjs';
...
// Catalogs often retain only the primary artist; provider details list every credit.
const creditedArtistMatches = (names, artist) => {
  const expected = normalize(artist);
  return Boolean(expected) && (normalize(names.join(' ')) === expected || names.some(name => normalize(name) === expected));
};
...
export function matchMetingRecording(song, hints) {
  ...
  return normalize(song?.name) === normalize(hints.title)
    && creditedArtistMatches((song?.artists || []).map(a => a.name), hints.artist)
...
export function createMetingBackend({ fetchImpl = fetch } = {}) {
  const mediaFetch = fetchImpl === fetch ? createMediaFetch() : fetchImpl;
  const matches = new Map(), inflight = new Map(), registered = new Map(), urls = new Map();
  let resolving = 0, streaming = 0, searching = 0;
  ...
  async function json(url, options = {}) {
    const response = await fetchImpl(url, { redirect: 'error', signal: AbortSignal.timeout(15_000), ...options });
  ...
  async function resolve(hints, { prefetch = false } = {}) {
    ...
    if (prefetch && resolving >= 1) throw fail('Background music lookup is busy.', 503);
    if (resolving >= 4) throw fail('Music lookup is busy. Please try again shortly.', 503);
    resolving++;
    const work = (async () => {
      const signal = AbortSignal.timeout(20_000);
      let lookupError;
      const searchQueries = [...new Set([`${artist} ${title}`, title])];
      for (const query of searchQueries) {
        let rows;
        try { rows = await json(metingUrl({ type: 'search', id: query, limit: '10' }), { signal }); }
        catch (error) { if (signal.aborted) throw error; lookupError = error; continue; }
        const ids = [...new Set((Array.isArray(rows) ? rows : []).slice(0, 10).filter(row =>
          normalize(row.name) === normalize(title) && creditedArtistMatches([String(row.artist || ''), ...String(row.artist || '').split('/')], artist)
        ).flatMap(row => {
          try {
        ...
        if (ids.length) {
          const details = await json(`https://music.163.com/api/song/detail/?ids=${encodeURIComponent(JSON.stringify(ids))}`, { signal });
      ...
      // Meting's hosted search can return an empty array while the provider's
      // own public catalog still has the recording. Search metadata directly;
      // keep exactly the same title, artist and duration requirements.
      try {
        const details = await json('https://music.163.com/api/search/get', {
          method: 'POST', signal,
          headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
          body: new URLSearchParams({ s: artist + ' ' + title, type: '1', limit: '10', offset: '0' }).toString()
        });
        const song = (Array.isArray(details.result?.songs) ? details.result.songs : []).slice(0, 10)
          .filter(song => /^\d{1,16}$/.test(String(song.id)) && matchMetingRecording(song, { title, artist, duration: hints.duration }))
          .sort((a,b) => Math.abs(a.duration / 1000 - Number(hints.duration || a.duration / 1000)) - Math.abs(b.duration / 1000 - Number(hints.duration || b.duration / 1000)))[0];
        if (song) {
          const id = String(song.id);
          const match = { mode: 'meting', id, title: song.name, durationSeconds: song.duration / 1000, streamUrl: '/api/nyxify/audio/' + id };
          put(registered, id, Date.now() + ttl); put(matches, key, { match, expires: Date.now() + ttl }); return match;
        }
  ...
  async function openAudio(id, range, signal, refresh = false) {
    ...
    for (let hop = 0; hop < 4; hop++) {
      ...
      const response = await (allowedMetingAudioUrl(target) ? mediaFetch : fetchImpl)(target, {
      ...
      if (response.status === 206) {
        const partial = /^bytes (\d+)-(\d+)\/(\d+)$/.exec(response.headers.get('content-range') || '');
        const requested = /^bytes=(\d+)-/.exec(range);
        const start = Number(partial?.[1]), end = Number(partial?.[2]), total = Number(partial?.[3]);
        if (!partial || !Number.isSafeInteger(total) || start > end || end >= total || total > maxBytes
          || (requested && start !== Number(requested[1]))
          || (response.headers.has('content-length') && length !== end - start + 1)) {
          await response.body.cancel(); throw fail('The music provider returned an invalid audio range.');
        }
      }
  ...
  async function stream(req, res) {
    ...
    if (range) {
      ...
      if ([start, end].some(n => n && !Number.isSafeInteger(Number(n)))
        || (start && end && Number(end) < Number(start)) || (!start && Number(end) === 0)) throw fail('Invalid audio range.', 416);
    }
  ...
  async function resolveId(id, hints, { prefetch = false } = {}) {
    if (!/^\d{1,16}$/.test(id)) throw fail('Invalid music recording.', 400);
    const key = JSON.stringify(['id', id, String(hints.title || '').slice(0,180), String(hints.artist || '').slice(0,120), Number(hints.duration) || 0]);
    if (inflight.has(key)) return inflight.get(key);
    const cached = matches.get(key);
    if (cached?.expires > Date.now() && registered.get(id) > Date.now()) return cached.match;
    if (resolving >= (prefetch ? 1 : 4)) throw fail('Music lookup is busy. Please try again shortly.', 503);
    resolving++;
    const task = (async () => {
      const details = await json(`https://music.163.com/api/song/detail/?ids=${encodeURIComponent(JSON.stringify([id]))}`);
      const song = (details.songs || []).find(song => String(song.id) === id && matchMetingRecording(song, hints));
      if (!song) throw fail('The selected recording is no longer available.', 404);
      put(registered, id, Date.now() + ttl);
      const match = { mode: 'meting', id, title: song.name, durationSeconds: song.duration / 1000, streamUrl: `/api/nyxify/audio/${id}` };
      put(matches, key, {match, expires:Date.now()+ttl});
      return match;
  ...
  async function search(query) {
    const key = String(query || '').trim().slice(0, 100);
    if (key.length < 2) return [];
    const cached = searchCache.get(key.toLowerCase());
    if (cached?.expires > Date.now()) return structuredClone(cached.rows);
    if (searchInflight.has(key.toLowerCase())) return searchInflight.get(key.toLowerCase());
    if (searching >= 2) throw fail('Music search is busy.', 503);
    searching++;
    const task = (async () => {
      const signal = AbortSignal.timeout(12000);
      const details = await json('https://music.163.com/api/search/get', {
        method: 'POST', signal, headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ s: key, type: '1', limit: '12', offset: '0' }).toString()
      });
      const candidates = (Array.isArray(details.result?.songs) ? details.result.songs : []).filter(song =>
        /^\d{1,16}$/.test(String(song.id)) && song.name && song.artists?.[0]?.name && song.duration > 45000 && song.duration <= 14400000
      ).slice(0, 6);
      const rows = new Array(candidates.length); let next = 0;
      await Promise.all([0, 1].map(async () => {
        while (next < candidates.length && !signal.aborted) {
          const index = next++, song = candidates[index], id = String(song.id);
          try {
            const response = await openAudio(id, 'bytes=0-4095', signal);
            // Check actual bytes, not just a catalog match or an HTTP 200 error page.
            const reader = response.body.getReader();
            let first;
            try { first = (await reader.read()).value; } finally { await reader.cancel().catch(() => {}); }
            if (!first?.length || !(first[0] === 0x49 && first[1] === 0x44 && first[2] === 0x33 || first[0] === 0xff && (first[1] & 0xe0) === 0xe0)) continue;
            put(registered, id, Date.now() + ttl);
            rows[index] = { id: `netease:${id}`, title: String(song.name).slice(0, 180), artist: String(song.artists[0].name).slice(0, 120),
              artistId: '', album: String(song.album?.name || '').slice(0, 160), albumId: '', cover: '/apps/nyxify/cover-placeholder.svg',
              catalog: 'netease', duration: Math.round(song.duration / 1000), audioAvailable: true };
          } catch { /* An unavailable recording must not displace working catalog results. */ }
        }
      }));
      const found = rows.filter(Boolean);
      put(searchCache, key.toLowerCase(), { rows: found, expires: Date.now() + (found.length ? 60000 : 10000) });
      return found;
    })().finally(() => { searching--; searchInflight.delete(key.toLowerCase()); });
    searchInflight.set(key.toLowerCase(), task);
    return task;
  }
  return { resolve, resolveId, search, stream };
}
