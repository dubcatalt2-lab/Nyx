...
function permitted(info, full) {
  return VIDEO.test(info?.id || '') && (!full || info.availability === 'public')
    ...
    // Embed permission is not native playback permission. The extractor and
    // public/non-DRM playback checks govern our own player, not an iframe flag.
...
export function createTubeCatalog(options = {}) {
  ...
  function prune(key) {
    ...
    cache.delete(key);
    if (!cache.size) cacheBytes = 0;
  }
  function remember(key, value, ttl) {
    const bytes = Buffer.byteLength(JSON.stringify(value));
    if (bytes > 8 * 1024 ** 2) return value;
    prune(key);
    // Only evict existing entries: accounting drift must never spin on an empty map.
    while (cache.size && (cache.size >= 160 || cacheBytes + bytes > 32 * 1024 ** 2)) prune(cache.keys().next().value);
  ...
  async function info(id, {refresh = false} = {}) {
    validateId(id);
    if (refresh) prune(`info:${id}`);
