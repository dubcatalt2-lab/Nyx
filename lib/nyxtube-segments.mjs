...
export const TUBE_SEGMENT_WORKERS = 4;
const WRITE_RESERVE = TUBE_SEGMENT_WORKERS * 2 * MAX_SEGMENT;
...
export function createTubeSegments({ root, files, leases, evict, info, choices, response, guard, now, record }) {
  ...
  async function read(media, format, start, end, signal) {
    ...
    for (let offset = start; offset <= end;) {
      ...
      for (let attempt = 0; attempt < 3; attempt++) {
        ...
        } catch (e) { if (signal.aborted || attempt === 2 || ['video', 'authentication', 'rate', 'stale'].includes(e.code)) throw e; }
      }
  ...
  function pump() {
    while (!closed && running < TUBE_SEGMENT_WORKERS && queue.length) {
  ...
  async function segment(token, kind, number, signal) {
    ...
    if (job?.controller.signal.aborted) { await job.promise.catch(() => {}); signal.throwIfAborted(); job = jobs.get(name); }
    if (!job) {
      ...
      job = { controller, consumers: 0, run: async () => {
        ...
        try {
          // Reserve every bounded writer plus temporary/publication overhead.
          await evict(WRITE_RESERVE);
          const disk = await statfs(root);
          if (disk.bavail * disk.bsize < 2 * 1024 * MB + WRITE_RESERVE) throw error('The server needs more free space before preparing video segments.');
          const refresh = async () => {
            entry.refresh ||= info(entry.id, true, controller.signal).then(media => {
          ...
          if (entry.refreshed < now() - 4 * 60000) await refresh();
          ...
          let range;
          try { range = await read(entry.media, track.format, first, last - 1, controller.signal); }
          catch (e) { if (e.code !== 'stale' || controller.signal.aborted) throw e; await refresh(); range = await read(entry.media, track.format, first, last - 1, controller.signal); }
