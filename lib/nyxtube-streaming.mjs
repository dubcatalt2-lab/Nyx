...
import { createTubeSegments, HLS_CACHE_NAME, TUBE_SEGMENT_WORKERS } from './nyxtube-segments.mjs';
...
export function extractionFailure(stderr = '') {
  ...
  if (/private video|members.only/i.test(stderr)) return new TubeError('video', 'This video requires access through its YouTube account or membership.', 422);
  if (/age.restricted|confirm your age/i.test(stderr)) return new TubeError('video', 'YouTube requires age verification for this video.', 422);
  if (/not available in your country|not made.*available in your country|geo.restricted/i.test(stderr)) return new TubeError('video', 'YouTube does not make this video available in the server’s region.', 422);
...
export function createTubeBackend(options = {}) {
  ...
  const cacheLimit = 5 * 1024 * MB, assetLimit = 1536 * MB, reserve = 2 * assetLimit + 16 * MB;
  const cacheIdleMs = 2 * 3600000;
  ...
  let writes = Promise.resolve(), eviction = Promise.resolve();
  ...
  async function initialize() {
    ...
    for (const name of await readdir(root)) {
      ...
      if (CACHE_NAME.test(name) || HLS_CACHE_NAME.test(name)) {
        const s = await stat(path);
        if (s.isFile() && s.size <= (HLS_CACHE_NAME.test(name) ? 16 * MB : assetLimit + 8 * MB) && s.mtimeMs > now() - cacheIdleMs) files.set(name, { path, size: s.size, touched: s.mtimeMs });
  ...
  function record(error) {
    if (['video', 'busy', 'expired'].includes(error?.code)) return;
    ...
    if (!error) { health.state='working'; health.authFailures=0; health.lastSuccess=health.lastChecked; health.lastError=null; cooldown=0; }
      ...
      const code = ['authentication','service','setup','rate'].includes(error.code) ? error.code : 'service';
      health.authFailures = code === 'authentication' ? health.authFailures + 1 : 0;
      health.lastError = code;
      health.state = health.authFailures >= 2 ? 'login_required' : code === 'setup' ? 'setup_required' : 'trouble';
      cooldown = now() + (code === 'authentication' || code === 'rate' ? 5 * 60000 : 30000);
    }
    persist();
  }
  async function status() {
    ...
    return { enabled, ...health, sessionPresent, cacheBytes: [...files.values()].reduce((n,f)=>n+f.size,0), cacheLimitBytes:cacheLimit, cachedVideos:new Set([...files.keys()].map(name=>HLS_CACHE_NAME.test(name)?name.slice(4,36):name)).size, activeJobs:active?1:segments.active+Number(segments.indexing), maxJobs:TUBE_SEGMENT_WORKERS+1, maxHeight:720, checkAvailableAt: new Date(Math.max(checkAfter,cooldown)).toISOString() };
  }
  function evict(required) {
    const work = eviction.catch(() => {}).then(() => evictFiles(required));
    eviction = work; return work;
  }
  async function evictFiles(required) {
    ...
    for (const [name,f] of [...files].sort((a,b)=>a[1].touched-b[1].touched)) {
      ...
      if (size+required<=cacheLimit && f.touched>now()-cacheIdleMs) continue;
      files.delete(name);
      try { await rm(f.path,{force:true}); }
      catch (error) { files.set(name,f); throw error; }
      size-=f.size;
    }
    if ([...files.values()].reduce((n,f)=>n+f.size,0)+required>cacheLimit) throw new TubeError('busy','The video cache is busy. Try again shortly.',429);
  }
  async function extract(id, signal, refresh = false) {
    if (!VIDEO_ID.test(id)) throw new TubeError('video','Invalid video ID.',400);
    if (options.videoInfo) {
      const info = await options.videoInfo(id, {refresh, signal});
      mediaChoices(info);
  ...
  async function mediaResponse(info, format, signal, range) {
    ...
    if(!response.ok) { await response.body?.cancel(); throw new TubeError(response.status===429?'rate':[403,410].includes(response.status)?'stale':'service','The video stream is temporarily unavailable.',502); }
  ...
  async function prepare(id,height) {
    ...
    guard(); if(active || checkPromise || inflight.size || segments.active || segments.indexing) throw new TubeError('busy','Another video is being prepared. Try again shortly.',429);
    ...
    job.promise=(async()=>{
      ...
      const timer=setTimeout(()=>job.controller.abort(),12*60000);
      try {
        await evict(reserve);
        ...
        if([choice.video,choice.audio].filter(Boolean).reduce((sum,f)=>sum+Math.max(0,Number(f.filesize)||0),0)>assetLimit) throw new TubeError('video','This video exceeds the 1.5 GiB per-video limit. Try a lower quality.',422);
        await mkdir(work,{mode:0o700});
        let received=0;
        async function download(format,name) {
          const size=Number(format.filesize || 0);
          if(size>assetLimit) throw new TubeError('video','This video exceeds the 1.5 GiB per-video limit. Try a lower quality.',422);
          ...
          while(true) {
            ...
            if(total>assetLimit) { await response.body?.cancel(); throw new TubeError('video','This video exceeds the 1.5 GiB per-video limit. Try a lower quality.',422); }
            let chunkBytes=0;
            const counter=new Transform({transform(chunk,enc,done){received+=chunk.length;chunkBytes+=chunk.length;done(received>assetLimit?new TubeError('video','This video exceeds the 1.5 GiB per-video limit. Try a lower quality.',422):null,chunk);}});
  ...
  const segments = createTubeSegments({ root, files, leases, evict, now, record, choices: mediaChoices, response: mediaResponse,
    info(id, refresh, signal) { if (refresh) metadata.delete(id); return extract(id, signal, refresh); },
    guard() { guard(); if (active || checkPromise) throw new TubeError('busy','Another video is being prepared. Try again shortly.',429); }
  });
  const segmented = fn => async (...args) => {
    await ready;
    if (!enabled || closed) throw new TubeError('setup', 'Native playback is not enabled.');
    try { return await fn(...args); }
    catch (error) {
      if (error instanceof TubeError) throw error;
      if (['video','busy','expired'].includes(error.code)) throw new TubeError(error.code, error.message, error.status);
      throw new TubeError('service', 'The video segment could not be prepared. Try again shortly.');
    }
  };
  const prepareSegments = segmented((id, height) => {
    if (!VIDEO_ID.test(id) || !QUALITY.has(height)) throw new TubeError('video','Choose a supported video and quality.',400);
    return segments.prepare(id, height);
  });
  const timer=enabled?setInterval(()=>{ if(active || closed)return; segments.sweep(); ready.then(()=>evict(0)).catch(()=>{}); for(const [key,job] of jobs) if(job.state!=='preparing'&&job.created<now()-10*60000)jobs.delete(key); },60000):null;
  timer?.unref();
  async function close() {closed=true;clearInterval(timer);active?.controller.abort();await segments.close();await active?.promise;await checkPromise;await ready.catch(()=>{});await eviction.catch(()=>{});await writes.catch(()=>{});}
  return {enabled,formats,prepare,prepareSegments,jobStatus,lease,status,check,close,
    playlist:segmented(segments.playlist), init:segmented(segments.init), segment:segmented(segments.segment)};
}
