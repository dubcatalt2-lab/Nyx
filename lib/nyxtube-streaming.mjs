...
import { mkdir, readdir, stat, statfs, readFile, writeFile, copyFile, chmod, rename, rm } from 'node:fs/promises';
...
export function mediaChoices(info) {
  if (info.availability !== 'public' || info.is_live || Number(info.age_limit) > 0 || !(info.duration > 0 && info.duration <= 3600)) throw new TubeError('video', 'Native playback supports public videos up to one hour.', 422);
...
function execute(binary, args, { timeout = 30000, signal, maxBytes = 8 * MB } = {}) {
...
export function createTubeBackend(options = {}) {
  ...
  const run = options.execute || execute, request = options.fetch || fetch;
  ...
  async function mediaResponse(info, format, signal, range) {
    ...
    const response=await request(publicMediaUrl(format.url),{headers,signal,redirect:'error'});
      ...
      try {
        ...
        async function download(format,name) {
          ...
          while(true) {
            ...
            await pipeline(Readable.fromWeb(response.body),counter,createWriteStream(join(work,name),{flags:offset?'a':'wx',mode:0o600}),{signal});
