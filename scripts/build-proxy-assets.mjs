...
export const proxyAssetNames=JSON.parse(await readFile(new URL('./proxy-asset-names.json',import.meta.url),'utf8'));
...
export async function scrambleProxyCode(source,{module=false}={}){
  ...
  const result=await minify(source,{
    ...
    mangle:{toplevel:module,eval:false,properties:false,nth_identifier:{get(n){if(!names.has(n))names.set(n,'_'+randomBytes(6).toString('hex'));return names.get(n);}}},
    format:{comments:/@license|@preserve|^!/,ascii_only:true}
  });
...
export async function buildProxyAssets(output){
  ...
  for(const [original,alias] of entries){
    ...
    if(original.endsWith('.wasm')){await writeFile(join(output,alias.slice(1)),bytes);continue;}
  ...
  for(const path of await walk(output)){
    ...
    if(proxyAssetNames['/'+path]||entries.some(([,alias])=>alias==='/'+path)||path.startsWith('assets/ugs/')||path.startsWith('assets/vendor/'))continue;
  ...
  await writeFile(join(output,'proxy-assets.json'),JSON.stringify({version:1,aliases:proxyAssetNames}));
