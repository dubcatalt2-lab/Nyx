...
assert.equal(chatSendDecision(state,'twenty first',time+50000).retryAfter,10);
state={};for(const text of ['hello','HELLO','he\u200bllo']){const result=chatSendDecision(state,text,time++);state=result.state;}
assert(state.until>time);assert.equal(chatSendDecision(state,'other',time).retryAfter,10);
...
const roles={member:'member',mod:'moderator',dev:'developer',manager:'manager',admin:'admin',co_owner:'co_owner',owner:'owner',support:'support'};
const burstChecks=[];
...
vm.runInContext(source.match(/^const nyxChatReactionCatalog=.*$/m)[0]+'\n'+source.match(/^const nyxChatReactionEmoji =.*$/m)[0],context);
vm.runInContext(source.slice(source.indexOf('function nyxChatReactionPayload('),source.indexOf('function nyxChatCustomRole(')),context);
vm.runInContext(source.slice(source.indexOf('app.post("/api/chat/messages/:scope/:messageId/reactions",'),source.indexOf('app.post("/api/activity/heartbeat",')),context);
...
try {
 ...
 const crossRoom=await send('member','new room','dm',{channel:undefined,conversationId:'private'});assert.equal(crossRoom.status,429);assert.equal(crossRoom.headers.get('retry-after'),'10');
 ...
 time+=61000;
 for(const uid of ['mod','dev','manager','admin','co_owner','owner']){
   records.set('nyxChatSendState/'+uid,{until:time+30000,recent:[]});
   const checks=burstChecks.length;
   for(let i=0;i<25;i++)assert.equal((await send(uid,'repeat staff','exempt-'+uid+'-'+i,i%2?{channel:undefined,conversationId:'private'}:{})).status,201);
   assert.equal(burstChecks.length,checks,'Staff bypass the legacy burst limiter too');
 }
 // A stale cached identity cannot bypass a fresh server-side demotion.
 records.set('nyxUserAdministration/mod',{role:'member'});
 assert.equal((await send('mod','demoted','demoted',{role:'owner',canModerate:true})).status,429);
 assert.equal(burstChecks.at(-1),'mod');
 time+=61000;
 for(let i=0;i<5;i++)assert.equal((await send('support','support '+i,'support-'+i,{canModerate:true})).status,201);
 const limited=await send('support','too fast','support-6');assert.equal(limited.status,429);assert.equal(limited.headers.get('retry-after'),'10');
 time+=10001;assert.equal((await send('support','after ten seconds','support-retry')).status,201);
 const reactionMessage='f'.repeat(40);records.set('nyxChatChannels/general/messages/'+reactionMessage,{reactions:[]});
 const react=emoji=>request('member','/api/chat/messages/general/'+reactionMessage+'/reactions',{emoji});
 for(const emoji of ['\u{1F680}','\u{1F98B}','\u{1F469}\u{1F3FD}\u200D\u{1F4BB}','\u{1F1EF}\u{1F1F5}']){let r=await react(emoji);assert.equal(r.status,200);assert((await r.json()).reactions.some(item=>item.emoji===emoji&&item.self));r=await react(emoji);assert.equal(r.status,200);assert(!(await r.json()).reactions.some(item=>item.emoji===emoji));}
 assert.equal((await react('<img src=x>')).status,400);assert.equal((await react('hello')).status,400);
 for(const emoji of Object.values(nyxEmojiMartData.emojis).slice(0,20).map(e=>e.skins[0].native))assert.equal((await react(emoji)).status,200);
 assert.equal((await react('\u{1F680}')).status,409);
 console.log('PASS full Unicode reactions, skin tones, flags, toggle removal and bounded reaction document size');
 console.log('PASS staff bypass across channels/DMs, existing cooldowns, legacy limiter, fresh demotion, lower-role spoof rejection and ten-second recovery');
