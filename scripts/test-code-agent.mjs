...
try {
 const page=await browser.newPage({viewport:{width:1440,height:900}});let answer={summary:'Created a Python program and edited JavaScript',files:[{language:'python',code:'print("created")'},{language:'javascript',code:'console.log("edited")'}]},request,hold=false,release;
 ...
 await page.route('**/api/nyx-ai',async r=>{request=r.request().postDataJSON();if(hold)await new Promise(resolve=>{release=resolve});await r.fulfill({json:{text:typeof answer==='string'?answer:JSON.stringify(answer)}})});
