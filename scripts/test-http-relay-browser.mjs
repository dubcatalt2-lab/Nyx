...
try{
 ...
 for(const brand of ['nyx','tutsi']){
  ...
  await context.addInitScript(()=>{localStorage.setItem('nyx.setupComplete','true');localStorage.setItem('nyx.tosAcceptedVersion','2026-07-30');localStorage.setItem('nyx.releaseNotes.2026-09-14-nyx-1.0.3.seen','2026-09-14-nyx-1.0.3');localStorage.setItem('nyx.httpBridge','true');localStorage.setItem('nyx.transport','libcurlRaw');localStorage.setItem('tutsi.customize.seen','1');localStorage.setItem('tutsi.settings.v1',JSON.stringify({transport:'libcurl',httpBridge:true,closePrevention:false}));window.WebSocket=class{constructor(){throw new Error('Native WebSockets disabled for HTTP test');}};});
  const page=await context.newPage();let batches=0,legacy=0;page.on('request',r=>{if(r.url().endsWith('/api/tutsi-relay/send-batch'))batches++;if(r.url().endsWith('/api/tutsi-relay/send'))legacy++;});
  ...
  assert(batches>0);assert.equal(legacy,0);
  console.log(`${brand}: real libcurl/Wispurr navigation over batched HTTP, ${batches} batches, ${Date.now()-start}ms; native WebSockets unavailable`);
  await context.close();
 }
