...
import {proxyAssetNames as names,rewriteProxyReferences,scrambleProxyCode} from './build-proxy-assets.mjs';
...
assert.notEqual(a.code,b.code);assert(!a.code.includes('localValue'));
...
for(const [original,alias] of Object.entries(names)){
  ...
  await access('dist'+alias);await access('dist'+original);
