...
const bridge='wss://tutsi.test/api/tutsi-relay/socket/';
assert.deepEqual(transportCandidates({httpBridge:false,autoRelay:false},{},localPage),['wss://tutsi.test/resources/live/']);
assert.deepEqual(transportCandidates({httpBridge:false,autoRelay:false,relay:bridge},{},localPage),['wss://tutsi.test/resources/live/']);
assert.deepEqual(transportCandidates({httpBridge:false,autoRelay:false,relay:urls[0]},{},localPage),[urls[0]]);
assert(!transportCandidates({httpBridge:false},{wispUrl:bridge,wispUrls:[bridge,...urls]},localPage).includes(bridge),'Disabled bridge cannot reappear through fallback or runtime configuration');
console.log('HTTP bridge opt-out, custom relay preservation and fallback exclusion passed');
