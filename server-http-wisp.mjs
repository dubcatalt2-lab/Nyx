...
export function installHttpWisp(app, {upstream, allowed, clientIp, banned = async () => false}) {
  ...
  router.post('/sessions', async (req, res) => {
    try {
      ...
      const ws = new WebSocket(target.url, {origin: target.origin, handshakeTimeout: 8000, maxPayload: maxQueue, perMessageDeflate: false});
      ...
      ws.on('message', (data, binary) => {
        ...
        s.frames.push(size, data); s.bytes += 4 + data.length; flush(s);
      });
  ...
  router.post('/send', express.raw({type: 'application/octet-stream', limit: '256kb'}), (req, res) => {
    ...
    if (s.ws.bufferedAmount + req.body.length > maxQueue) { dispose(s); return res.sendStatus(429); }
    s.sequence++;
